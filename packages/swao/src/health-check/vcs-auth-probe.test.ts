...
import { buildVcsAuthProbe, runGitLsRemote, buildGitConfigEnv } from './vcs-auth-probe.js';
...
// #2749: buildGitConfigEnv -- unit tests for the GIT_CONFIG_* env var builder.
describe('buildGitConfigEnv (#2749)', () => {
  it('returns empty object when no proxy or ssl_verify override is given', () => {
    expect(buildGitConfigEnv()).toEqual({});
    expect(buildGitConfigEnv(undefined, undefined)).toEqual({});
    expect(buildGitConfigEnv(undefined, true)).toEqual({});
  });

  it('builds http.proxy entry when proxy is provided', () => {
    const env = buildGitConfigEnv('http://proxy.corp.example:8080');
    expect(env['GIT_CONFIG_COUNT']).toBe('1');
    expect(env['GIT_CONFIG_KEY_0']).toBe('http.proxy');
    expect(env['GIT_CONFIG_VALUE_0']).toBe('http://proxy.corp.example:8080');
  });

  it('builds http.sslVerify=false entry when ssl_verify is false', () => {
    const env = buildGitConfigEnv(undefined, false);
    expect(env['GIT_CONFIG_COUNT']).toBe('1');
    expect(env['GIT_CONFIG_KEY_0']).toBe('http.sslVerify');
    expect(env['GIT_CONFIG_VALUE_0']).toBe('false');
  });

  it('builds both entries when proxy and ssl_verify:false are given', () => {
    const env = buildGitConfigEnv('http://proxy.corp.example:3128', false);
    expect(env['GIT_CONFIG_COUNT']).toBe('2');
    expect(env['GIT_CONFIG_KEY_0']).toBe('http.proxy');
    expect(env['GIT_CONFIG_VALUE_0']).toBe('http://proxy.corp.example:3128');
    expect(env['GIT_CONFIG_KEY_1']).toBe('http.sslVerify');
