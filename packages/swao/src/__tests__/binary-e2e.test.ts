...
import { cpSync, existsSync, mkdtempSync, openSync, closeSync, readdirSync, readFileSync, rmSync, statSync } from 'fs';
...
// On Windows, TerminateProcess only kills the direct child; grandchildren that
// inherited the pipe write-ends keep them open and spawnSync blocks forever on
// pipe-EOF. Using file-backed stdio (not pipes) avoids this: spawnSync waits
// for process exit via WaitForSingleObject, not pipe drain -- it returns as
// soon as the direct child is gone. We then tree-kill any grandchildren.
// See vcs-auth-probe.ts runGitLsRemote for the established pattern.
...
function runBinary(
  ...
  const base = join(tmpdir(), `swao-e2e-${Date.now()}`);
  ...
  try {
    const r = spawnSync(bin, args, { cwd, timeout: 60_000, stdio: ['ignore', outFd, errFd], env });
    pid = r.pid;
    status = r.status;
  } finally {
  ...
  // Always tree-kill after each run: direct child has already exited but
  // grandchildren (playwright, git, etc.) may still hold the binary open.
  ...
  try { rmSync(errPath, { force: true }); } catch { /* ignore */ }
  return { stdout, stderr, status: status ?? -1 };
}
...
describe.skipIf(!hasBinary)('binary E2E -- swao-enterprise-win.exe', () => {
  ...
  // #1704: lz.assessment.start + lz.assessment.complete events emitted by CLI path
  // #2288: renamed from lz.assess.* to lz.assessment.* for consistency with portfolio-events.
  it('assess --type landing-zone-catalog emits lz.assessment.start and lz.assessment.complete to app-events (#1704 #2288)', () => {
    const { status } = run([
      'assess', '--app', 'sovereign-health',
      '--type', 'landing-zone-catalog',
      '--lz-cat-targets', 'stackit:eu01',
      '--lz-frameworks', 'GDPR',
    ]);
    expect(status).toBe(0);
    ...
    const events = lines.map(l => { try { return JSON.parse(l) as { code?: string }; } catch { return null; } }).filter(Boolean);
    const hasStart = events.some(e => e?.code === 'lz.assessment.start');
  ...
  it('health-check runs without crashing', () => {
    ...
    // spawnSync timeout maps to -1 via runBinary; treat as skip on slow CI
    if (status === -1) return;
  ...
  it('publish generates HTML publication for sovereign-health', () => {
    // sprint-128: publish (HTML Report) requires Consultant or Enterprise license.
    // Tests run with SANDBOX_HOME (no license) so the binary returns 0 (licensed)
    // or 2 (Community -- licence gate). Both outcomes are valid in this suite.
    const { stdout, stderr, status } = run(
    ...
    if (status === 0) {
      ...
      expect(combined.toLowerCase()).toMatch(/consultant|enterprise|license|licence/);
    }
  });
