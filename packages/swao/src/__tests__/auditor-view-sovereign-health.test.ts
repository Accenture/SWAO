...
// #2124: skip all tests when the portfolio-workspace fixture is not committed.
const hasFixture = existsSync(SOURCE_WSP);
...
describe.skipIf(!hasFixture)('Auditor view against the sovereign-health golden fixture (#0174)', () => {
  ...
  it('coverage table includes medium-severity negative (DATA-02 Art. 17(3)(b))', () => {
    // Layer 4 report standardisation (5f99d335) restricted drill-down to high/critical.
    // DATA-02 (severity: medium) is captured in the negative-by-severity coverage table
    // but no longer rendered with its ID in the drill-down section.
    const out = formatViewAuditor(data, SOVEREIGN_HEALTH_WSP);
    expect(out).toMatch(/Negative outcomes by severity/);
    expect(out).toMatch(/medium\s+\d+/);
  });
