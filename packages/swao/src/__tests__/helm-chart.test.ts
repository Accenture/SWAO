...
// #2435: skip entire suite when ops/helm/swao/ has not yet been created.
const chartDirExists = existsSync(CHART_DIR);

// -- Chart file presence --------------------------------------------------

describe.skipIf(!chartDirExists)('Helm chart file presence', () => {
...
describe.skipIf(!chartDirExists)('Chart.yaml content', () => {
  const chart = chartDirExists ? readFileSync(join(CHART_DIR, 'Chart.yaml'), 'utf-8') : '';
...
describe.skipIf(!chartDirExists)('values.yaml content', () => {
  const values = chartDirExists ? readFileSync(join(CHART_DIR, 'values.yaml'), 'utf-8') : '';
...
describe.skipIf(!chartDirExists)('templates/job.yaml content', () => {
  const job = chartDirExists ? readFileSync(join(CHART_DIR, 'templates/job.yaml'), 'utf-8') : '';
...
// -- helm lint (skipped when helm not installed or chart dir absent) ------

describe.skipIf(!chartDirExists)('helm lint', () => {
