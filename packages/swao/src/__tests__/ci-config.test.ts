...
describe('Tracker integrity (#0120)', { sequential: true }, () => {
  ...
  it('every open issue has state: open frontmatter field', () => {
    ...
    for (const file of files) {
      ...
      expect(fm['state'] ?? fm['status'], `${file}: missing state/status field`).toBe('open');
    }
  }, 30_000); // open issues grow with each sprint; Windows readFileSync overhead exceeds 5s default
