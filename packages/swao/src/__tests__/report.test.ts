...
// ---------------------------------------------------------------------------
// formatLlmText (#2273): LLM Assessment text report
// ---------------------------------------------------------------------------
describe('swao report --type llm text format (#2273)', () => {
  const baseLlmInput = {
    appId:    'sovereign-health',
    runTs:    '2026-08-29T10-00-00',
    legs: [
      { id: 'leg-a', connector: 'openai', model: 'gpt-4o',  primary: true  },
      { id: 'leg-b', connector: 'ollama', model: 'llama3',  primary: false },
    ],
    weights: { Quality: 0.5, Reliability: 0.5 },
    groups: [
      { group: 'Quality',     score: { 'leg-a': 82, 'leg-b': 71 }, rank: { 'leg-a': 1, 'leg-b': 2 }, light: {} },
      { group: 'Reliability', score: { 'leg-a': 78, 'leg-b': 80 }, rank: { 'leg-a': 2, 'leg-b': 1 }, light: {} },
    ],
    scores: { 'leg-a': 80, 'leg-b': 75.5 },
    ranks:  { 'leg-a': 1, 'leg-b': 2 },
    findings: [
      { id: 'f1', severity: 'warn', leg: 'leg-a', pass_id: 'pass-01', message: 'PII token detected in response.' },
    ],
  };

  it('contains app id and run timestamp in header', () => {
    const text = formatLlmText(baseLlmInput);
    expect(text).toContain('sovereign-health');
    expect(text).toContain('2026-08-29T10-00-00');
  });

  it('lists all legs with connector/model', () => {
    const text = formatLlmText(baseLlmInput);
    expect(text).toContain('openai/gpt-4o');
    expect(text).toContain('ollama/llama3');
    expect(text).toContain('primary');
  });

  it('includes score weights', () => {
    const text = formatLlmText(baseLlmInput);
    expect(text).toContain('Quality');
    expect(text).toContain('50%');
  });

  it('includes group scores and ranks', () => {
    const text = formatLlmText(baseLlmInput);
    expect(text).toContain('Group Scores');
    expect(text).toContain('rank #1');
  });

  it('includes final scores', () => {
    const text = formatLlmText(baseLlmInput);
  ...
  it('includes findings section', () => {
    ...
    expect(text).toContain('PII token detected');
  });
  ...
  // #2402: LLM YAML path must NOT pass the LLM data to formatYamlReport (which
  // expects a ReportData with blockers/topFindings arrays and crashes with
  // "Cannot read properties of undefined (reading 'map')"). Use dump() directly.
  it('#2402: LLM YAML dump does not crash and contains expected keys', () => {
    const data = {
      app: baseLlmInput.appId,
      run_ts: baseLlmInput.runTs,
      legs: baseLlmInput.legs,
      weights: baseLlmInput.weights,
      groups: baseLlmInput.groups,
      final: { score: baseLlmInput.scores, rank: baseLlmInput.ranks },
      findings: baseLlmInput.findings,
    };
    expect(() => dump(data, { lineWidth: 120 })).not.toThrow();
    const parsed = load(dump(data, { lineWidth: 120 })) as Record<string, unknown>;
    expect(parsed['app']).toBe('sovereign-health');
    expect(Array.isArray(parsed['legs'])).toBe(true);
    expect(parsed['final']).toBeDefined();
  });
});