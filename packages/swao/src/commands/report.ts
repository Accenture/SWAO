...
export function generateReport(workspaceAppDir: string, appId: string): ReportData {
  ...
  if (spine) {
    ...
    if (spineLz.primary) landingZone = normalizeDashes(String(spineLz.primary));
  }
  ...
  if (existsSync(passesDir)) {
    ...
    for (const file of files) {
      ...
      if (file.startsWith('09-synth') && parsed.assessment) {
        ...
        if (a['landing_zone']) landingZone = normalizeDashes(String(a['landing_zone']));
  ...
  return {
    ...
    // #2432: surface challenge status for the report header note.
    challengeStatus: challengeFindings.length > 0
      ? 'included'
      : (existsSync(challengeBase) ? 'not-run' : 'not-available'),
  };
}
...
// #2273: LLM Assessment text report (Community+). Mirrors LZ text report style.
interface LlmTextReportInput {
  appId: string;
  runTs: string;
  engagement?: EngagementMeta;
  legs:     { id: string; connector: string; model: string; primary: boolean }[];
  weights:  Record<string, number>;
  groups:   { group: string; score: Record<string, number | null>; rank: Record<string, number | null>; light: Record<string, string> }[];
  scores:   Record<string, number | null>;
  ranks:    Record<string, number | null>;
  findings: { id: string; severity: string; leg?: string; pass_id?: string; message: string }[];
}

export function formatLlmText(d: LlmTextReportInput): string {
  const title = `SWAO LLM Assessment Report -- ${d.appId}`;
  ...
  // Providers compared
  lines.push('LLM Providers Compared', '-'.repeat(30));
  for (const leg of d.legs) {
    const primary = leg.primary ? '  (primary)' : '';
    lines.push(`  ${leg.id.padEnd(10)} ${leg.connector}/${leg.model}${primary}`);
  }
  lines.push('');

  // Score weights
  lines.push('Score Weights', '-'.repeat(30));
  for (const [grp, w] of Object.entries(d.weights)) {
    lines.push(`  ${grp.padEnd(20)} ${(w * 100).toFixed(0)}%`);
  }
  lines.push('');

  // Group scores
  if (d.groups.length > 0) {
    lines.push('Group Scores', '-'.repeat(30));
  ...
  if (d.findings.length > 0) {
    ...
    for (const f of d.findings) {
      const loc = [f.leg, f.pass_id].filter(Boolean).join(' / ');
      const prefix = loc ? `${loc}: ` : '';
      lines.push(`  [${f.severity}] ${prefix}${f.message}`);
    }
    lines.push('');
  }
...
export function formatViewAuditor(data: ReportData, wspDir: string): string {
  ...
  const drillSignals = signals
    .filter((s) => s.outcome === 'negative' && (s.severity === 'critical' || s.severity === 'high'))
    .slice(0, 25);
...
export function registerReport(program: Command, deps: ReportDeps): void {
  program
      ...
      if (value === 'pdf') {
        try {
          ...
          if (e instanceof LicenseTierError) {
            printLicenseGateError(e.feature ?? 'swao report --format pdf', e.required, e.current);
          }
    ...
    .action(async (opts: { app?: string; workspace?: string; format: string; view: string; output?: string; run?: string; portfolio: boolean; allViews: boolean; type: string }) => {
      ...
      if (opts.portfolio) {
        ...
        try {
          ...
          if (err instanceof LicenseTierError) {
            printLicenseGateError(err.feature ?? 'swao report --portfolio', err.required, err.current);
          }
        ...
        const workspaceRoot = opts.workspace
          ...
          : findWorkspace(process.cwd()); // #0137 / #2185: no silent cwd fallback
        if (!workspaceRoot) {
      ...
      // --- Format validation (#2442) ---
      // 'all' generates text + yaml + json in one step; handled per assessment type below.
      const VALID_FORMATS = ['text', 'yaml', 'json', 'pdf', 'html', 'all'] as const;
      if (opts.format && !VALID_FORMATS.includes(opts.format as typeof VALID_FORMATS[number])) {
        console.error(`[error] Unknown format "${opts.format}". Valid options: ${VALID_FORMATS.join(', ')}`);
        process.exit(1);
      }
      ...
      if (opts.format === 'pdf') {
        ...
        try {
          ...
          if (err instanceof LicenseTierError) {
            printLicenseGateError(err.feature ?? 'swao report --format pdf', err.required, err.current);
          }
          if (err instanceof LicenseLimitError) {
            printLicenseLimitError(err);
          }
      ...
      if (opts.type === 'landing-zone-catalog') {
        ...
        if (opts.format === 'yaml') {
          ...
          lzOutExt = 'json';
          // #2270: use explicit snake_case mapping so JSON and YAML field names are consistent
          const base = {
            app: lzData.appId,
            assessed_at: lzData.assessedAt,
            assessment_mode: lzData.assessmentMode,
            ...(lzData.engagement ? { engagement: lzData.engagement } : {}),
            targets: lzData.targets.map(t => ({
              provider: t.provider,
              region: t.region,
              overall: t.overall,
              assessment_mode: t.assessmentMode,
              sovereignty_active: t.sovereigntyActive,
              sovereignty_statement: t.sovereigntyStatement,
              ...(t.blockerCategory ? { blocker_category: t.blockerCategory } : {}),
              items: t.items,
            })),
        ...
        // #2442/#2580: --format all generates text + yaml + json in one step for LZ reports.
        if (opts.format === 'all') {
          const lzTextContent = formatLzText(lzData);
          const txtOut = lzBranding.text.length > 0 ? lzBranding.text.join('\n') + '\n' + lzTextContent : lzTextContent;
          const txtPath = join(reportsLzDir, `${lzRunTsPrefix}lz-report.txt`);
          writeFileSync(txtPath, txtOut, 'utf-8');
          console.log(`[ok]  Report (text) written to ${txtPath}`);
          const yamlBody = formatLzYaml(lzData);
          const yamlOut = lzBranding.yaml ? lzBranding.yaml + yamlBody : yamlBody;
          const yamlPath = join(reportsLzDir, `${lzRunTsPrefix}lz-report.yaml`);
          writeFileSync(yamlPath, yamlOut, 'utf-8');
          console.log(`[ok]  Report (yaml) written to ${yamlPath}`);
          const lzJsonBase = {
            app: lzData.appId, assessed_at: lzData.assessedAt, assessment_mode: lzData.assessmentMode,
            ...(lzData.engagement ? { engagement: lzData.engagement } : {}),
            targets: lzData.targets.map(t => ({
              provider: t.provider, region: t.region, overall: t.overall,
              assessment_mode: t.assessmentMode, sovereignty_active: t.sovereigntyActive,
              sovereignty_statement: t.sovereigntyStatement,
              ...(t.blockerCategory ? { blocker_category: t.blockerCategory } : {}),
              items: t.items,
            })),
          ...
          const lzJsonWrapped = lzBranding.data ? { _generated_for: lzBranding.data, ...lzJsonBase } : lzJsonBase;
          const jsonPath = join(reportsLzDir, `${lzRunTsPrefix}lz-report.json`);
          writeFileSync(jsonPath, JSON.stringify(lzJsonWrapped, null, 2), 'utf-8');
          console.log(`[ok]  Report (json) written to ${jsonPath}`);
          return;
        }
        ...
        if (opts.format === 'pdf') {
          ...
          // #2291 #2294: trigger_source lets operators distinguish intentional re-runs from double-triggers.
          logPortfolio('info', 'report.generate.start', `LZ report generation started -- ${reportsExpected} PDF(s)`, { context: { app: opts.app, report_type: 'landing-zone-catalog', reports_expected: reportsExpected, trigger_source: 'user_action' } });
      ...
      // --- LLM Assessment report branch (#1531, #2273) ---
      // text/yaml/json: Community+; pdf: Consultant+ only.
      if (opts.type === 'llm') {
        ...
        const rawRun   = raw['run'] as Record<string, unknown> | undefined;
        ...
        if (opts.format === 'pdf') {
          ...
          const llmPdfStartMs = Date.now();
          logPortfolio('info', 'report.generate.start', `LLM report generation started`, { context: { app: opts.app, report_type: 'llm', output: llmOutPath, trigger_source: 'user_action' } });
          ...
          logPortfolio('info', 'report.generate.complete', `LLM PDF report generation complete`, { context: { app: opts.app, report_type: 'llm', format: 'pdf', reports_generated: 1, duration_ms: Date.now() - llmPdfStartMs } });
          console.log(`[ok]  Report written to ${llmOutPath}`);
          return;
        }

        // #2273: text/yaml/json are Community+ -- no tier gate
        const legs     = (raw['legs']    as LlmPdfArgs['legs']    | undefined) ?? [];
        const weights  = (raw['weights'] as LlmPdfArgs['weights'] | undefined) ?? {};
        const groups   = (raw['groups']  as LlmPdfArgs['groups']  | undefined) ?? [];
        const findings = (raw['findings'] as LlmPdfArgs['findings'] | undefined) ?? [];
        const scores   = rawFinal.score ?? {};
        const ranks    = rawFinal.rank  ?? {};
        ...
        // #2405: read engagement block from wsp.yaml (same source as App/LZ reports)
        let llmEngagement: EngagementMeta | undefined;
        try {
          const appWspDir = join(workspaceAppDir, 'wsp');
          const appRunDir = resolveRunDir(appWspDir);
          const appSpine = loadYaml(join(appRunDir, 'wsp.yaml')) as Record<string, unknown> | null;
          if (appSpine) {
            const spineEng = (appSpine['engagement'] ?? {}) as Record<string, unknown>;
            if (spineEng['name'] || spineEng['client_code'] || spineEng['partnership_lead'] || spineEng['start_date']) {
              llmEngagement = {
                name:             typeof spineEng['name'] === 'string'             ? (spineEng['name'] as string)             : undefined,
                client_code:      typeof spineEng['client_code'] === 'string'      ? (spineEng['client_code'] as string)      : undefined,
                partnership_lead: typeof spineEng['partnership_lead'] === 'string' ? (spineEng['partnership_lead'] as string) : undefined,
        ...
        } catch { /* engagement metadata is optional -- silently omit */ }

        if (opts.format === 'text') {
          const body = formatLlmText({ appId: opts.app!, runTs, engagement: llmEngagement, legs, weights, groups, scores, ranks, findings });
          const llmOutPath = opts.output ? resolve(opts.output) : join(reportLlmDir, `${runTs}-llm-comparison.txt`);
          const llmTextStartMs = Date.now();
          logPortfolio('info', 'report.generate.start', `LLM text report generation started`, { context: { app: opts.app, report_type: 'llm', output: llmOutPath, trigger_source: 'user_action' } });
          writeFileSync(llmOutPath, body, 'utf-8');
          logPortfolio('info', 'report.generate.ok', `LLM report written: ${llmOutPath}`, { context: { app: opts.app, file: llmOutPath } });
          // #2060: emit complete to close the start/complete sequence pair.
          logPortfolio('info', 'report.generate.complete', `LLM text report generation complete`, { context: { app: opts.app, report_type: 'llm', format: 'text', reports_generated: 1, duration_ms: Date.now() - llmTextStartMs } });
          console.log(`[ok]  Report written to ${llmOutPath}`);
          return;
        }
        if (opts.format === 'yaml') {
          const data = { schema_version: '1.0', app: opts.app, run_ts: runTsIso, ...(llmEngagement ? { engagement: llmEngagement } : {}), ...(rawRun ? { run: rawRun } : {}), legs: legsWithDisplayName, weights, groups, final: { score: scores, rank: ranks }, findings };
          const body = dump(data, { lineWidth: 120 });
          const llmOutPath = opts.output ? resolve(opts.output) : join(reportLlmDir, `${runTs}-auditor.yaml`);
          const llmYamlStartMs = Date.now();
          logPortfolio('info', 'report.generate.start', `LLM yaml report generation started`, { context: { app: opts.app, report_type: 'llm', output: llmOutPath, trigger_source: 'user_action' } });
          writeFileSync(llmOutPath, body, 'utf-8');
          logPortfolio('info', 'report.generate.ok', `LLM report written: ${llmOutPath}`, { context: { app: opts.app, file: llmOutPath } });
          // #2060: emit complete to close the start/complete sequence pair.
          logPortfolio('info', 'report.generate.complete', `LLM yaml report generation complete`, { context: { app: opts.app, report_type: 'llm', format: 'yaml', reports_generated: 1, duration_ms: Date.now() - llmYamlStartMs } });
          console.log(`[ok]  Report written to ${llmOutPath}`);
          return;
        }
        if (opts.format === 'json') {
          const data = { schema_version: '1.0', app: opts.app, run_ts: runTsIso, ...(llmEngagement ? { engagement: llmEngagement } : {}), ...(rawRun ? { run: rawRun } : {}), legs: legsWithDisplayName, weights, groups, final: { score: scores, rank: ranks }, findings };
          const llmOutPath = opts.output ? resolve(opts.output) : join(reportLlmDir, `${runTs}-auditor.json`);
          const llmJsonStartMs = Date.now();
          logPortfolio('info', 'report.generate.start', `LLM json report generation started`, { context: { app: opts.app, report_type: 'llm', output: llmOutPath, trigger_source: 'user_action' } });
          writeFileSync(llmOutPath, JSON.stringify(data, null, 2), 'utf-8');
          logPortfolio('info', 'report.generate.ok', `LLM report written: ${llmOutPath}`, { context: { app: opts.app, file: llmOutPath } });
          // #2060: emit complete to close the start/complete sequence pair.
          logPortfolio('info', 'report.generate.complete', `LLM json report generation complete`, { context: { app: opts.app, report_type: 'llm', format: 'json', reports_generated: 1, duration_ms: Date.now() - llmJsonStartMs } });
          console.log(`[ok]  Report written to ${llmOutPath}`);
          return;
        }
        // #2601: --format all for LLM reports (mirrors App + LZ parity).
        // TUI decomposes 'all' before spawning, so this path is CLI-direct only.
        if (opts.format === 'all') {
          const allStartMs = Date.now();
          const data = { schema_version: '1.0', app: opts.app, run_ts: runTsIso, ...(llmEngagement ? { engagement: llmEngagement } : {}), ...(rawRun ? { run: rawRun } : {}), legs: legsWithDisplayName, weights, groups, final: { score: scores, rank: ranks }, findings };
          const txtPath  = join(reportLlmDir, `${runTs}-llm-comparison.txt`);
          const yamlPath = join(reportLlmDir, `${runTs}-auditor.yaml`);
          const jsonPath = join(reportLlmDir, `${runTs}-auditor.json`);
          logPortfolio('info', 'report.generate.start', 'LLM all-format report generation started', { context: { app: opts.app, report_type: 'llm', format: 'all', trigger_source: 'user_action' } });
          writeFileSync(txtPath,  formatLlmText({ appId: opts.app!, runTs, engagement: llmEngagement, legs, weights, groups, scores, ranks, findings }), 'utf-8');
          console.log(`[ok]  Report written to ${txtPath}`);
          writeFileSync(yamlPath, dump(data, { lineWidth: 120 }), 'utf-8');
          console.log(`[ok]  Report written to ${yamlPath}`);
          writeFileSync(jsonPath, JSON.stringify(data, null, 2), 'utf-8');
          console.log(`[ok]  Report written to ${jsonPath}`);
          logPortfolio('info', 'report.generate.complete', 'LLM all-format report generation complete', { context: { app: opts.app, report_type: 'llm', format: 'all', reports_generated: 3, duration_ms: Date.now() - allStartMs } });
          return;
        }
        console.error(`[error] Unknown format "${opts.format}" for LLM Assessment report. Valid formats: text, yaml, json, pdf, all`);
        process.exit(1);
      }
      ...
      if (opts.allViews) {
        ...
        logPortfolio('info', 'report.generate.start', `App report generation started -- ${allViewKeys.length} view(s)`, { context: { app: opts.app, report_type: 'application', format: opts.format, reports_expected: allViewKeys.length, trigger_source: 'user_action' } });
        let allViewsGenerated = 0;
        for (const v of allViewKeys) {
          // #2061: per-file render timing for diagnosing slow PDF/text renders.
          ...
          if (opts.format === 'pdf') {
            ...
            allViewsGenerated++;
            logPortfolio('info', 'report.generate.ok', `App report written: ${vPath}`, { context: { app: opts.app, file: vPath, view: v, duration_ms: Date.now() - viewStartMs } });
            console.log(`[ok]  Report written to ${vPath}`);
          } else if (opts.format === 'yaml') {
            // #2269: support yaml format under --all-views
            const body = formatYamlReport(data);
            const yamlOut = allViewsBranding.yaml ? allViewsBranding.yaml + body : body;
            const vPath = join(reportsDir, `${runTsPrefix}${fileName}.yaml`);
            writeFileSync(vPath, yamlOut, 'utf-8');
            allViewsGenerated++;
            logPortfolio('info', 'report.generate.ok', `App report written: ${vPath}`, { context: { app: opts.app, file: vPath, view: v, duration_ms: Date.now() - viewStartMs } });
            console.log(`[ok]  Report written to ${vPath}`);
          } else if (opts.format === 'json') {
            // #2269: support json format under --all-views
            const base = data as unknown as Record<string, unknown>;
            const wrapped = allViewsBranding.data ? { _generated_for: allViewsBranding.data, ...base } : base;
            const vPath = join(reportsDir, `${runTsPrefix}${fileName}.json`);
            writeFileSync(vPath, JSON.stringify(wrapped, null, 2), 'utf-8');
            allViewsGenerated++;
            logPortfolio('info', 'report.generate.ok', `App report written: ${vPath}`, { context: { app: opts.app, file: vPath, view: v, duration_ms: Date.now() - viewStartMs } });
            console.log(`[ok]  Report written to ${vPath}`);
          } else {
      ...
      // #2442: --format all generates text + yaml + json in one step for App reports.
      if (opts.format === 'all') {
        const reportsDir = join(wspDir, 'reports-app');
        mkdirSync(reportsDir, { recursive: true });
        const viewName = viewFilenameToken(canonicalView);
        const textRenderer = VIEW_RENDERERS[canonicalView] ?? VIEW_RENDERERS['application-architect']!;
        const textRendered = textRenderer(data, wspDir);
        const textContent = branding.text.length > 0 ? branding.text.join('\n') + '\n' + textRendered : textRendered;
        const textPath = join(reportsDir, `${runTsPrefix}${viewName}.txt`);
        writeFileSync(textPath, textContent, 'utf-8');
        console.log(`[ok]  Report (text) written to ${textPath}`);
        const auditData = buildAuditorReport(data, wspDir);
        const yamlBody = dump(auditData, { lineWidth: 120 });
        const yamlContent = branding.yaml ? branding.yaml + yamlBody : yamlBody;
        const yamlPath = join(reportsDir, `${runTsPrefix}auditor.yaml`);
        writeFileSync(yamlPath, yamlContent, 'utf-8');
        console.log(`[ok]  Report (yaml) written to ${yamlPath}`);
        const jsonBase = branding.data ? { _generated_for: branding.data, ...auditData } : auditData;
        const jsonPath = join(reportsDir, `${runTsPrefix}auditor.json`);
        writeFileSync(jsonPath, JSON.stringify(jsonBase, null, 2), 'utf-8');
        console.log(`[ok]  Report (json) written to ${jsonPath}`);
        return;
      }
