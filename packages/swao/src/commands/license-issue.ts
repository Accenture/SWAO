...
export function issueLicense(opts: IssueOptions): IssueResult {
  ...
  // --- fp ---
  if (!/^[0-9a-f]{8,16}$/.test(opts.fp)) {
    throw new Error(`Invalid --fp "${opts.fp}". Expected 8 or 16 lowercase hex characters (use the value from 'swao license' or 'swao license request').`);
  }
