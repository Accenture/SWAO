...
import { licenseStateToJson, buildRequestToken, buildRequestLines, buildRemovalToken, communityStatusText } from './license.js';
...
describe('communityStatusText -- higher-tier licence display (#2664)', () => {
  it('shows "Previous licence expired" when exp is in the past', () => {
    const guard0 = LicenseGuard.load();
    const fp = guard0.state.fingerprint;
    writeLicenseKey({ v: 1, tier: 'enterprise', licensee: 'Accenture', email: 'a@example.com', exp: '2020-01-01', assessment_limit: null, fp, iat: '2019-01-01' });
    const state = LicenseGuard.load().state;
    const lines = communityStatusText(state).join('\n');
    expect(lines).toContain('Previous licence expired: 2020-01-01');
    expect(lines).not.toContain('Higher-tier licence on file');
  });

  it('shows "Higher-tier licence on file" when exp is in the future (binary cap, not true expiry)', () => {
    const guard0 = LicenseGuard.load();
    const fp = guard0.state.fingerprint;
    writeLicenseKey({ v: 1, tier: 'enterprise', licensee: 'Helmut Schindlwick', email: 'h@accenture.com', exp: '2099-12-31', assessment_limit: null, fp, iat: '2026-09-04' });
    const state = LicenseGuard.load().state;
    const lines = communityStatusText(state).join('\n');
    expect(lines).toContain('Higher-tier licence on file (expires 2099-12-31');
    expect(lines).toContain('Use the Enterprise or Consultant binary');
    expect(lines).not.toContain('Previous licence expired');
  });

  it('same fix applies to Consultant licence capped by Community binary', () => {
    const guard0 = LicenseGuard.load();
    const fp = guard0.state.fingerprint;
    writeLicenseKey({ v: 1, tier: 'consultant', licensee: 'Test Org', email: 't@example.com', exp: '2099-12-31', assessment_limit: 100, fp, iat: '2026-09-04' });
    const state = LicenseGuard.load().state;
    const lines = communityStatusText(state).join('\n');
    expect(lines).toContain('Higher-tier licence on file (expires 2099-12-31');
    expect(lines).not.toContain('Previous licence expired');
...
describe('buildRequestToken', () => {
  it('returns a valid base64url token with correct fields', () => {
    ...
    expect(decoded.fp).toBe('abc123def456abc1');
    expect(decoded.requested_tier).toBe('consultant');
    expect(decoded.iat).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });

  it('stores full fingerprint (not truncated) so activate round-trip succeeds', () => {
    const token = buildRequestToken('ffffffffffffffff', 'enterprise');
    const decoded = JSON.parse(Buffer.from(token, 'base64url').toString('utf-8'));
    expect(decoded.fp).toBe('ffffffffffffffff');
...
describe('LicenseGuard.activate', () => {
  function buildPayload(overrides: Partial<LicensePayload> = {}): LicensePayload {
    const fp = LicenseGuard.load().state.fingerprint;
    return {
      v: 1,
      tier: 'consultant',
      licensee: 'Test Org',
...
// ---------------------------------------------------------------------------
// buildRemovalToken (#2315)
// ---------------------------------------------------------------------------

describe('swao license remove token', () => {
  it('buildRemovalToken returns a valid base64url-decodable JSON with correct fields', () => {
    const rawBytes = Buffer.from('{"key":"testkey","activated_at":"2026-08-01","tier":"consultant","exp":"2027-08-01","licensee":"Test User"}');
    const token = buildRemovalToken('abc123def456abc1', 'consultant', 'Test User', rawBytes);
    const decoded = JSON.parse(Buffer.from(token, 'base64url').toString('utf-8')) as Record<string, unknown>;
    expect(decoded['v']).toBe(1);
    expect(decoded['type']).toBe('license_removal');
    expect(decoded['fp']).toBe('abc123def456abc1');
    expect(decoded['tier']).toBe('consultant');
    expect(decoded['licensee']).toBe('Test User');
    expect(decoded['removed_at']).toMatch(/^\d{4}-\d{2}-\d{2}$/);
    expect(typeof decoded['sig']).toBe('string');
    expect((decoded['sig'] as string)).toHaveLength(16);
  });

  it('sig field is first 16 hex chars of HMAC-SHA256', () => {
    const rawBytes = Buffer.from('{"key":"testkey","tier":"consultant"}');
    const token = buildRemovalToken('abc123def456abc1', 'consultant', 'Test', rawBytes);
    const decoded = JSON.parse(Buffer.from(token, 'base64url').toString('utf-8')) as Record<string, unknown>;
    // Re-derive the expected HMAC using the decoded removed_at (not a fresh Date()) to avoid
    // midnight-boundary flakiness.
    const hmacKey = createHash('sha256').update(rawBytes).digest();
    const expectedSig = createHmac('sha256', hmacKey)
      .update(`${decoded['fp']}|${decoded['tier']}|${decoded['removed_at']}`)
      .digest('hex')
      .slice(0, 16);
    expect(decoded['sig']).toBe(expectedSig);
  });

  it('two calls with same inputs produce same token (deterministic)', () => {
    const rawBytes = Buffer.from('{"key":"testkey","tier":"consultant"}');
    const token1 = buildRemovalToken('abc123def456abc1', 'consultant', 'Test', rawBytes);
    const token2 = buildRemovalToken('abc123def456abc1', 'consultant', 'Test', rawBytes);
    // Both calls must produce identical tokens -- date-only granularity ensures
    // same-day calls are always deterministic.
...
// ---------------------------------------------------------------------------
// #2652: legacy licence file left behind after removal triggers migration shim
// ---------------------------------------------------------------------------
// The migration shim in LicenseGuard.load() (core/license-guard.ts) copies the
// legacy `~/.swao-license.json` to the new AppData path whenever the new path is
// absent but the legacy exists.  If `license remove` only deleted the new path,
// the shim would restore the old key on the next load -- causing
// fingerprint_mismatch when the restored key does not match this machine.
//
// Fix (license.ts): delete _paths.legacyLicencePath in addition to licensePath.
// These tests verify:
//   (a) the bug scenario: legacy-only key triggers fingerprint_mismatch
//   (b) the fixed state: both deleted => community tier returned cleanly

describe('#2652 -- legacy licence path cleared on removal', () => {
  it('(a) legacy-only scenario: migration shim triggers fingerprint_mismatch for wrong-fp key', () => {
    // Write state with a known fingerprint.
    ...
    // Write a legacy licence with a DIFFERENT fingerprint (wrong machine).
    const wrongFpPayload: LicensePayload = {
      fp:        '00000000',  // does not match state fingerprint prefix
      tier:      'consultant',
      exp:       '2099-01-01',
      licensee:  'Other Machine',
    };
    const wrongKey = buildLicenseKey(wrongFpPayload);
    writeFileSync(_paths.legacyLicencePath, JSON.stringify({
      key:          wrongKey,
    ...
    // Primary licence absent: migration shim copies legacy -> primary.
    // Then verifyKey finds fp mismatch.
    expect(() => LicenseGuard.load()).toThrow(LicenseInvalidError);
    try { LicenseGuard.load(); } catch (e) {
      expect((e as LicenseInvalidError).code).toBe('fingerprint_mismatch');
    }
  });

  it('(b) fixed state: both paths absent => community tier, no error', () => {
    writeFileSync(_paths.statePath, JSON.stringify({
      first_run: '2026-01-01',
      assessment_count: 0,
      fingerprint: 'abc1234567890abc',
    ...
    // Simulate the fix: both licence paths are absent.
    // (In production, registerLicenseRemoveCommand deletes both.)
    // legacyLicencePath is already absent (not written in this test).
    expect(existsSync(_paths.licensePath)).toBe(false);
    expect(existsSync(_paths.legacyLicencePath)).toBe(false);
