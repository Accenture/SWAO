...
// #2154: reduced from 80 to 60 to lower peak memory and reduce the likelihood
// of OOM / Chromium renderer crashes on large sites with heavy SPA pages.
// Operators can raise via assessment.crawler_max_turns in .swao.yml.
...
// #2322: Inline RFC 4648 base32 decoder (no external dependency).
// Strips padding and ignores spaces/dashes that appear in some TOTP seed strings.
...
// Generate a 6-digit TOTP code per RFC 6238 (HMAC-SHA1, 30s time step).
function generateTotp(seed: string): string {
  const key = decodeBase32(seed);
  const counter = Math.floor(Date.now() / 1000 / 30);
  const buf = Buffer.alloc(8);
  // Upper 32 bits (time step rarely exceeds 32-bit range within normal use)
...
// OTP input selectors in preference order (handles most TOTP step-up pages).
const OTP_SELECTORS = [
...
async function attemptFormLogin(
  ...
  try {
    ...
    // #2322: TOTP step-up -- if an OTP input field is visible after the credential
    // submit (MFA page or inline MFA challenge), fill it with the generated code.
    if (mfaSeed) {
      let otpField = null;
      ...
      if (otpField) {
        const totpCode = generateTotp(mfaSeed);
        ...
        if (!otpSubmitted) await otpField.press('Enter');
        await page.waitForURL(
          (url) => !/\/(login|signin|auth\/login|2fa|mfa|otp|verify)\b/i.test(new URL(url).pathname),
          { timeout: 12000 },
        ).catch(() => {});
        console.warn('[info] Playwright: TOTP step-up completed');
      }
    }
...
export class PlaywrightCrawlProvider implements CrawlProvider {
  ...
  async crawl(config: CrawlConfig, _workspaceAppDir: string): Promise<CrawlResult> {
    ...
    try {
      ...
      // #2154: cap sitemap pre-seed at 2*maxTurns to avoid an unbounded queue
      // that keeps all URLs in memory even when we can only visit maxTurns of them.
      const SITEMAP_CAP = maxTurns * 2;
      try {
        const origin = new URL(targetUrl).origin;
        const sitemapUrls = await fetchSitemapUrls(ctx, origin);
        let added = 0;
        for (const u of sitemapUrls) {
          if (queue.length - 1 >= SITEMAP_CAP) break;
      ...
      while (queue.length > 0 && screens.length < maxTurns) {
        ...
        if (!loginAttempted && authType === 'form' && username && password) {
          loginAttempted = true;
          const loginResult = await attemptFormLogin(page, username, password, mfaSeed);
