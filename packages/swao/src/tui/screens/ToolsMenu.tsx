...
export type ToolsTarget = Extract<MenuTarget, 'license' | 'credentials' | 'help' | 'lz-catalogue-update' | 'ingest' | 'support-bundle' | 'chat'>;

interface ToolsItem {
  key: string;
  label: string;
  detail: string;
  // #2417: when true, show [Consultant+] marker on Community/Consultant binaries
  consultantGated?: boolean;
  // #2778: when true, show [Enterprise] marker on non-Enterprise binaries
...
const ITEMS: ToolsItem[] = [
  ...
  { key: '3', label: 'Update LZ Catalogue', detail: 'refresh provider service catalogues',              target: 'lz-catalogue-update', consultantGated: true },
  ...
  // #2778 -- Multi-turn portfolio chat; Enterprise only.
  { key: '6', label: 'Chat with Portfolio',  detail: 'multi-turn LLM conversation about assessments',       target: 'chat', enterpriseGated: true },
  { key: '7', label: 'Help',                 detail: 'quick reference + troubleshooting',                  target: 'help'             },
];

const TOOLS_INFO: Record<ToolsTarget, ToolsInfo> = {
  license: {
    what:   'Licence management: view status, activate a key, or request a new licence.',
    use:    `Community: unlimited (Apache-2.0). Consultant: PDF export + Terraform. Enterprise: portfolio mode + challenge agents. For upgrades, see ${SWAO_LANDING_URL}`,
  ...
  help: {
    ...
    use:    `Open for a menu reference, quick-start steps, or common error diagnostics (fetch failed, no workspace found, VCS clone failed, blank reports). See: ${SWAO_LANDING_URL}`,
  ...
  ingest: {
    ...
    use:    'Run before `swao assess` with a large document set. The assessment will use the fresh manifest and skip re-processing. Re-run at any time after adding files to ingestion/.',
  ...
  chat: {
    ...
    use:    `Enterprise only. Requires a configured LLM gateway (run \`swao setup\`). The MCP server auto-starts to load portfolio context. History saved to wsp/chat/. See: ${SWAO_LANDING_URL}`,
  ...
  'lz-catalogue-update': {
    ...
    use:    'Requires Consultant+ licence. No cloud credentials needed -- AWS, Azure, GCP, and STACKIT use public APIs. Refreshed files go to wsp/inputs/catalogs/lz-catalogues/ and override the bundled copy.',
...
export function ToolsMenu({ onSelect, onBack }: ToolsMenuProps) {
  ...
  // #2659: load licence state at render time so the badge reflects the active
  // tier rather than the module-load-time env var (which may not be set yet).
  ...
  useInput((input, key) => {
    if (guidanceOpenRef.current && !key.upArrow && !key.downArrow && !key.escape && !key.return) return;
        ...
        {ITEMS.map((item, i) => {
          ...
          return (
            <Box key={item.key}>
              ...
              {/* #2659: show [Consultant+] only when active licence is below Consultant */}
              {item.consultantGated && !isAllowed(licenseState, 'consultant') && (
                <Text color="yellow">{'  '}[Consultant+]</Text>
              )}
              {/* #2778: show [Enterprise] only when active licence is below Enterprise */}
              {item.enterpriseGated && !isAllowed(licenseState, 'enterprise') && (
                <Text color="magenta">{'  '}[Enterprise]</Text>
              )}
