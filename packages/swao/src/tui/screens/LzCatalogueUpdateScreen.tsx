  ...
  useEffect(() => {
    ...
    const appendLines = (data: Buffer) => {
      ...
      if (chunks.length > 0) {
        ...
        // #0912: detect "updated:" / "written:" lines to advance progress bar.
        // #2623: curated providers emit a "source.mode curated -- no automated adapter"
        // skip message instead of "updated:" -- count those too so the bar advances.
        const completions = chunks.filter(l =>
          /^\s*(updated|written):/.test(l) ||
          /source\.mode.*curated.*no automated adapter/i.test(l)
        );
      ...
      >
        ...
        {phase === 'running' && (
          <Box flexDirection="column" marginTop={1}>
            ...
            />
            <LiveOutput lines={lines} maxLines={10} maxWidth={100} label="Output" />
          </Box>
        )}
        {phase === 'done' && (
          <Box flexDirection="column" marginTop={1}>
        ...
        details={[
          {
            label: 'When to use',
            value:
              'Requires Consultant+ licence. No cloud credentials needed -- AWS, Azure, GCP, and STACKIT use public APIs. Refreshed files go to wsp/inputs/catalogs/lz-catalogues/ and override the bundled copy.',
          },
          {
            label: 'Deliverables',
            value:
