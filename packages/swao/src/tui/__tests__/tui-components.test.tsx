...
describe('ToolsMenu', () => {
  ...
  // #2778: key 6 is now Chat with Portfolio (Enterprise); Help moved to key 7.
  it('calls onSelect with chat target when key 6 pressed (#2778)', async () => {
    const onSelect = vi.fn();
    const { stdin } = render(<ToolsMenu onSelect={onSelect} onBack={noop} />);
  ...
  it('calls onSelect with help target when key 7 pressed (#2778)', async () => {
    const onSelect = vi.fn();
    const { stdin } = render(<ToolsMenu onSelect={onSelect} onBack={noop} />);
    await pressKey(stdin, '7');
...
// ---------------------------------------------------------------------------
// IngestScreen (#2185 null-workspace guard)
// ---------------------------------------------------------------------------
describe('IngestScreen', () => {
  it('renders without crashing when no workspace found', async () => {
    const { lastFrame } = render(<IngestScreen onBack={noop} />);
    // Give the useEffect a tick to run.
    await new Promise<void>(r => setTimeout(r, 60));
    expect((lastFrame() ?? '').length).toBeGreaterThan(0);
  });

  it('shows no-workspace warning when findWorkspace returns null (#2185)', async () => {
    const { lastFrame } = render(<IngestScreen onBack={noop} />);
    await new Promise<void>(r => setTimeout(r, 60));
    const frame = lastFrame() ?? '';
    expect(frame).toContain('No SWAO workspace found');
    expect(frame).toContain('Workspace Setup');
  });

  it('transitions to done state immediately when no workspace found', async () => {
    const { lastFrame } = render(<IngestScreen onBack={noop} />);
    await new Promise<void>(r => setTimeout(r, 60));
    const frame = lastFrame() ?? '';
    // done state shows the press-to-return footer hint
    expect(frame).toContain('Press Enter');
  });
});

// ---------------------------------------------------------------------------
// LensesScreen null-workspace save guard (#2185)
// ---------------------------------------------------------------------------
describe('LensesScreen null-workspace', () => {
  it('shows no-workspace message when saving with no workspace found', async () => {
    const onBack = vi.fn();
    const { lastFrame, stdin } = render(<LensesScreen onBack={onBack} />);
    // findWorkspace mock returns null -- press Enter to attempt save.
    await pressKey(stdin, '\r');
    const frame = lastFrame() ?? '';
    expect(frame).toContain('No workspace found');
  });
});
