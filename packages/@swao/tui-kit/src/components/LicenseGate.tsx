...
export function LicenseGate({
  ...
  // Match GuidanceBox / Header width: capped at 100 display columns.
  ...
  return (
    <Box flexDirection="column" padding={1} borderStyle="single" borderColor="yellow" width={width}>
      ...
      {/* #2617: show expiry context when the tier dropped due to licence expiry. */}
      {state.expiredTier && (
        <Box marginTop={1}>
          <Text color="red">
            Your {TIER_LABEL[state.expiredTier as LicenseTier] ?? state.expiredTier} licence expired on {state.exp ?? 'unknown'} -- renew to restore access.
          </Text>
        </Box>
      )}
      <Box marginTop={1}>
