...
export interface LicenseStateView {
  ...
  exp?: string;
  /** Set when a paid tier expired and the session downgraded to Community (#2632). */
  expiredTier?: string;
}
...
export function licenseStatusColor(state: LicenseState): LicenseStatusColor {
  ...
  // Red: already expired, or within 3 days of expiry.
  if (daysLeft != null && daysLeft <= DAYS_RED) {
    return { state: 'red', inkColor: 'red', dim: false };
  }

  // Amber: within 14 days of expiry.
...
/**
 * Human-readable budget label for the status line. Always shows the lifetime
 * assessment count as "N assessments performed". The assessment_limit field
 * in the licence payload is a legacy issuance artefact -- no tier has a hard
 * assessment cap (D-05 revised), so "remaining" is not surfaced to the user.
 */
export function formatBudget(state: LicenseState): string {
  const used = state.assessmentCount;
  return `${used} assessments performed`;
}
...
export function formatExpiry(state: LicenseState): string | null {
  ...
  if (days < 0) {
    // #2607: use "inactive" -- avoids implying "expired" for manually-removed keys
    return `previous licence (inactive) ${state.exp} (${-days} days ago)`;
  }
