...
// #2366: formatLlmGatewayLine must use probe.ok as the authoritative status,
// not the bracketed message prefix. A [WARNING] message on a failed probe must
// produce FAIL output so the TUI and the NDJSON log agree.
describe('formatLlmGatewayLine -- #2366 ok-vs-message-prefix alignment', () => {
  it('shows FAIL when ok=false and message has [WARNING] prefix', () => {
    const line = formatLlmGatewayLine({ ok: false, message: '[WARNING] gateway schema invalid' });
    expect(line).toContain('FAIL');
    expect(line).not.toContain('WARN');
  });

  it('shows ok when ok=true and message has [PASS] prefix', () => {
    const line = formatLlmGatewayLine({ ok: true, message: '[PASS] 2 connector(s) valid' });
    expect(line).toContain('ok');
  });

  it('shows FAIL when ok=false with no bracketed prefix', () => {
    const line = formatLlmGatewayLine({ ok: false, message: 'connection refused' });
    expect(line).toContain('FAIL');
  });
  ...
  // not WARN -- consistent with NDJSON ok=true from the #2392 fix.
  it('shows ok (not WARN) when ok=true and message carries secondary warning note (#2222 #2392)', () => {
    const msg = '[PASS] live ping OK -- connector \'anthropic\'; 5 connector(s) discovered; 1 secondary/leg probe warning(s): secondary: model \'google/gemini-flash-latest\' rejected by the platform -- check the model id';
    const line = formatLlmGatewayLine({ ok: true, message: msg });
    expect(line).toContain('ok');
    expect(line).not.toContain('WARN');
