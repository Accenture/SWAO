...
import { readFileSync, existsSync, mkdirSync, writeFileSync, unlinkSync, openSync, closeSync, readdirSync, copyFileSync, statSync } from 'fs';
...
function treeKillSync(pid: number): void {
...
function runSwao(args: string[], cwd?: string): { stdout: string; stderr: string; ok: boolean } {
  // File-backed stdio avoids the Windows pipe-EOF deadlock: spawnSync with
  // pipe stdio hangs forever when grandchildren hold inherited write-ends after
  // TerminateProcess kills only the direct child (MCP spawns playwright and git
  // as grandchildren). File FDs don't block WaitForSingleObject on exit.
  const { cmd, cmdArgs } = resolveSpawn(hostDeps, args);
  const base = join(tmpdir(), `swao-mcp-${Date.now()}`);
  ...
  return { stdout, stderr, ok: status === 0 };
}

// MCP server version, set by startMcpServer via the version option (#2718).
let mcpVersion = '';

// Build the WELCOME string lazily so it picks up mcpVersion after server start.
...
---------------
  ...
  swao_read_challenge        Read existing challenge YAML(s) already on disk without re-running.
...
export const SWAO_MCP_TOOLS = [
      ...
      properties: {
        ...
        agent: {
          ...
          enum: [
            ...
            'finops-lead',
            'programme-manager',
            // Legacy aliases (mapped to canonical before CLI invocation)
            'technical',
            'exec',
            'compliance',
            'finops',
          ...
          description:
            'Stakeholder perspective. Canonical IDs: application-architect (default), ' +
            'business-owner, grc-compliance-officer, finops-lead, programme-manager. ' +
            'Legacy aliases (technical, exec, compliance, finops, migration-manager) ' +
            'are accepted and mapped to their canonical equivalents.',
        },
      },
      required: ['app_id'],
    },
  ...
  {
    name: 'swao_read_challenge',
    description:
      'Read existing stakeholder challenge YAML(s) already written to disk without re-running. ' +
      'Use this to retrieve previously generated challenge findings for any persona after a ' +
      'swao_challenge run or a TUI challenge session. Omit agent to return all personas at once.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        app_id: {
          type: 'string',
          description: 'Application identifier (matches folder name under apps/).',
        },
        workspace_path: {
          type: 'string',
          description: 'Absolute path to the portfolio workspace root.',
        ...
        agent: {
          ...
          enum: [
            ...
            'finops',
            'migration-manager',
            // LZ challenge personas
            'lzca-ciso-security',
            'lzca-lz-architect',
            'lzca-procurement',
            'lzca-sovereignty-grc',
          ],
          description: 'Stakeholder persona to read. Omit to return all available challenge YAMLs.',
        },
        challenge_type: {
          type: 'string',
          enum: ['app', 'lz'],
          description: 'Whether to read app-level (challenge-app/) or landing-zone (challenge-lz/) challenges. Default: app.',
        },
      },
      required: ['app_id'],
    },
  ...
  // #2772: Chat history tool stub -- lists chat sessions from wsp/chat/.
  {
    name: 'swao_chat_history',
    description:
      'List SWAO Chat Bridge sessions stored in wsp/chat/. ' +
      'Returns session filenames, creation timestamps, and per-session turn counts. ' +
      'Use to answer "what chat sessions exist?", "show me recent conversations", ' +
      'or "how many chat turns have been recorded?".',
    inputSchema: {
      type: 'object' as const,
      properties: {
        workspace_path: { type: 'string', description: 'Absolute path to the portfolio workspace root.' },
...
function resolveWorkspace(input: ToolInput): string | undefined {
  ...
  // #2719: fall back to the workspace path saved by the most recent swao setup/init.
  const saved = readDefaultWorkspace();
...
export function runSwaoAssessAsync(
  ...
  return new Promise((resolveOuter) => {
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      // child.kill() only terminates the direct child; grandchildren holding
      // inherited pipe write-ends keep them open so 'close' never fires.
      // Tree-kill releases all descendants, which closes the pipes and lets
      // 'close' fire so the Promise resolves.
      if (child.pid) treeKillSync(child.pid);
    }, ASSESS_TIMEOUT_MS);
...
// #2772: Chat history listing tool -- reads wsp/chat/*.ndjson session files.
export function handleChatHistory(input: ToolInput): string {
  const workspace = resolveWorkspace(input);
  if (!workspace) return 'Error: workspace_path required or could not be resolved.';
  const maxLimit = Math.min(Number(input.limit ?? 20), 100);
  const chatDir = join(workspace, 'wsp', 'chat');
  if (!existsSync(chatDir)) return `No chat sessions found. Chat directory does not exist: ${chatDir}`;
  let files: string[];
  try { files = readdirSync(chatDir).filter(f => f.endsWith('.ndjson')).sort().reverse(); }
  catch { return 'Error reading chat directory.'; }
  if (files.length === 0) return 'No chat sessions recorded yet. Start a session with SWAO-Chat.ps1.';
  const rows: string[] = [
    `Chat sessions in ${chatDir} (${files.length} total, showing ${Math.min(files.length, maxLimit)}):`,
    '',
    'Session                         Turns  Last turn',
    '-------------------------------- ------ ----------------------------------------',
  ];
  for (const f of files.slice(0, maxLimit)) {
    const fPath = join(chatDir, f);
    let turns = 0;
    let lastTs = '';
    try {
      ...
      for (const line of lines) {
        try {
          ...
          if (e['role'] === 'user' || e['role'] === 'assistant') turns++;
          if (typeof e['ts'] === 'string') lastTs = e['ts'];
        } catch { /* skip */ }
      }
    } catch { /* skip */ }
    const sid = f.replace('.ndjson', '').slice(0, 32).padEnd(32);
    rows.push(`${sid} ${String(turns).padStart(6)} ${lastTs}`);
  }
  return rows.join('\n');
}
...
// #2720: the CLI challenge command accepts canonical persona IDs only.
// Legacy aliases from the MCP schema are mapped here before spawning.
const CHALLENGE_ALIAS_MAP: Record<string, string> = {
...
function handleChallenge(input: ToolInput): string {
  const appId = String(input.app_id ?? '');
  const workspace = resolveWorkspace(input);
  let rawAgent = input.agent ? String(input.agent) : '';
  if (rawAgent && CHALLENGE_ALIAS_MAP[rawAgent]) rawAgent = CHALLENGE_ALIAS_MAP[rawAgent];
  const agent = rawAgent ? ['--agent', rawAgent] : [];
  const args = ['challenge', '--app', appId, ...agent, '--report'];
  const r = runSwao(args, workspace);
  if (!r.ok) {
    return (r.stdout + (r.stderr ? `\n[stderr]\n${r.stderr}` : '')).trim();
  }
  // #2721: --report writes YAML to disk and prints "[challenge] Report written to <path>".
  // Extract the actual file path from the stdout prefix before reading content.
  const pathMatch = r.stdout.match(/[Rr]eport written to (.+)/);
  const filePath = pathMatch ? pathMatch[1].trim() : r.stdout.trim();
  if (filePath && existsSync(filePath)) {
    try {
      const content = readFileSync(filePath, 'utf-8');
      return `Challenge output saved to: ${filePath}\n\n${content}`;
    } catch {
      return `Challenge output saved to: ${filePath}\n\n(Error reading output file)`;
    }
  }
  return filePath || '(no output)';
}

// #2723: read challenge YAMLs already on disk without re-running the LLM pass.
function handleReadChallenge(input: ToolInput): string {
  const appId = String(input.app_id ?? '');
  if (!appId) return 'Error: app_id is required.';

  const workspace = resolveWorkspace(input);
  if (!workspace) return 'Error: workspace_path is required (or set a default workspace via swao setup).';
  const challengeType = input.challenge_type ? String(input.challenge_type) : 'app';
  const subfolder = challengeType === 'lz' ? 'challenge-lz' : 'challenge-app';
  const challengeDir = join(workspace, 'apps', appId, 'wsp', subfolder);

  if (!existsSync(challengeDir)) {
    return `No challenge outputs found at ${challengeDir}. Run swao_challenge (or use the TUI challenge menu) first.`;
  }

  let rawAgent = input.agent ? String(input.agent) : '';
  if (rawAgent && CHALLENGE_ALIAS_MAP[rawAgent]) rawAgent = CHALLENGE_ALIAS_MAP[rawAgent];

  const prefix = challengeType === 'lz' ? 'LZCA_' : 'AA_';

  const files = readdirSync(challengeDir)
    .filter(f => f.endsWith('.yaml') && f.startsWith(prefix))
    .sort();

  if (files.length === 0) {
    return `No challenge YAML files found in ${challengeDir}.`;
  }

  const targets = rawAgent
    ? files.filter(f => f === `${prefix}${rawAgent}.yaml`)
    : files;

  if (targets.length === 0) {
    return `No challenge YAML found for agent "${rawAgent}" in ${challengeDir}. Available: ${files.join(', ')}`;
  }
...
function buildMcpServer(): Server {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    let text: string;
    switch (name) {
      case 'swao':                        text = buildWelcome();                   break;
      ...
      case 'swao_read_challenge':         text = handleReadChallenge(input);       break;
      ...
      case 'swao_chat_history':           text = handleChatHistory(input);           break;
      default:
...
export async function startMcpServer(
  opts: { http?: boolean; port?: number; workspace?: string; version?: string } = {},
  deps?: McpHostDeps,
): Promise<void> {
  // Set version before any transport connects so buildWelcome() returns the
  // correct string on the first swao tool call (#2718).
  if (opts.version) mcpVersion = opts.version;
