...
// #2725: swao mcp is Enterprise-only per feature-licence-tier-matrix F-105.
// The gate runs before any transport connects so stderr goes to the terminal,
// not into the stdio JSON-RPC stream.
export function registerMcp(program: Command, deps: McpHostDeps, version = ''): void {
  program
    .command('mcp')
    .description(
      'Start the SWAO MCP server (Enterprise). ' +
    ...
    .action(async (opts: { http?: boolean; port: string; workspace?: string }) => {
      LicenseGuard.load().requireTier('enterprise', { feature: 'swao mcp' });
      ...
      await startMcpServer(
        { http: opts.http ?? false, port: parseInt(opts.port, 10), workspace: opts.workspace, version },
        deps,
      );
    });
}
