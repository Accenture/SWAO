...
export interface IngestedFile {
  ...
  /** 'none' when file type has no text extraction capability (e.g. image formats). */
  extraction?: 'none';
  /** Operator-visible warning tag stored in manifest (e.g. 'image-no-text-extraction'). */
  warning?: string;
}
...
export async function runIngestPrePass(opts: IngestPrePassOptions): Promise<IngestManifest | null> {
  ...
  for (const rel of relFiles) {
    ...
    // Images (#2343, supersedes #1495 Option A): route to intake/ and warn.
    // Images contain no extractable text; operator can add a .description.txt sidecar.
    if (IMAGE_EXTS.has(ext)) {
      warn(`[warn] INGEST: ${fileName} -- image format has no text extraction; add a .description.txt sidecar to include this content in LLM pass coverage`);
    }
    ...
    // Images are always forced to intake/ (no classifier override) per #2343.
    const subdir = IMAGE_EXTS.has(ext)
      ? 'intake'
      : prev
    ...
    if (IMAGE_EXTS.has(ext)) {
      ...
      entry.warning = 'image-no-text-extraction';
    }
    files.push(entry);
  }
