...
import { PASS_DESCRIPTIONS } from './assess/shared/passDescriptions.js';
import { ChallengePromptPhase } from './assess/shared/ChallengePromptPhase.js';
import { RunningPhase } from './assess/shared/RunningPhase.js';
import { PreflightLlmPhase } from './assess/shared/PreflightLlmPhase.js';
import { AppPickerPhase } from './assess/shared/AppPickerPhase.js';
import type { LocalConnectorInfo } from './assess/shared/types.js';
...
type Phase =
  ...
  // #2329: API token entry for an LLM gateway connector
  ...
  | 'input-app-lz-region'
  // #2328: pre-flight LLM connectivity check before starting the run
  | 'preflight-llm'
  | 'running'
...
// PASS_DESCRIPTIONS imported from ./assess/shared/passDescriptions.js
...
// #2324: derive the provider-scoped credential key for a VCS token.
// Mirrors vcsTokenKey() in swao/src/providers/vcs/auth.ts; inlined to avoid
// a cross-package import from this guest module.
...
// LocalConnectorInfo moved to assess/shared/types.ts (#2371).
  ...
  for (const entry of entries.sort()) {
    try {
      const raw = loadYaml(readFileSync(join(wsDir, entry), 'utf-8')) as {
        connector?: {
          id?: string; name?: string; base_url?: string; protocol?: string;
          models?: { default?: string };
          auth?: { credential_key?: string; env_var?: string };
        };
      } | null;
      ...
      out.push({
        id,
        name: c?.name ?? id,
        credentialKey: c?.auth?.credential_key,
        envVar: c?.auth?.env_var,
        baseUrl: c?.base_url ?? '',
        protocol: c?.protocol ?? '',
        defaultModel: c?.models?.default ?? '',
      });
    } catch { /* skip invalid connector files */ }
  }
  return out;
}

// #2351: built-in LLM connectors always shown in the credential hub API tokens section.
// Derived from bundled seed YAMLs; only connectors with a credential_key or env_var are listed.
// Exported for testing.
export const BUILTIN_API_CONNECTORS: LocalConnectorInfo[] = [
  { id: 'anthropic',  name: 'Anthropic Claude', credentialKey: 'anthropic-api-key',  envVar: 'SWAO_ANTHROPIC_API_KEY', baseUrl: '', protocol: '', defaultModel: '' },
  { id: 'bedrock',    name: 'Amazon Bedrock',   credentialKey: undefined,            envVar: 'AWS_ACCESS_KEY_ID',      baseUrl: '', protocol: '', defaultModel: '' },
  { id: 'openrouter', name: 'OpenRouter',        credentialKey: 'openrouter-api-key', envVar: 'SWAO_OPENROUTER_API_KEY', baseUrl: '', protocol: '', defaultModel: '' },
];

// #2351: enumerate connectors for the credential hub API tokens section.
// Returns: built-in connectors + workspace connectors (wsp/inputs/llm-gateway/) + connectors
// from <workspace>/llm-gateways/ (simple credential reference YAMLs).
// Exported for testing.
export function listApiTokenConnectors(workspaceRoot?: string): LocalConnectorInfo[] {
  const out: LocalConnectorInfo[] = [...BUILTIN_API_CONNECTORS];
  const seen = new Set<string>(out.map(c => c.id));

  // Workspace connectors from wsp/inputs/llm-gateway/ not already in the built-in list
  for (const c of listLocalConnectors(workspaceRoot)) {
    if (!seen.has(c.id)) { seen.add(c.id); out.push(c); }
  }

  if (!workspaceRoot) return out;

  // Additional connectors from <workspace>/llm-gateways/ (simple or full connector format)
  const gwDir = join(workspaceRoot, 'llm-gateways');
  if (!existsSync(gwDir)) return out;
  let gwFiles: string[] = [];
  try { gwFiles = readdirSync(gwDir).filter(f => (f.endsWith('.yaml') || f.endsWith('.yml')) && !f.startsWith('_')); } catch { return out; }
  for (const f of gwFiles.sort()) {
    try {
      const parsed = loadYaml(readFileSync(join(gwDir, f), 'utf-8')) as Record<string, unknown> | null;
      if (!parsed) continue;
      const connector = parsed['connector'] as { id?: string; name?: string; base_url?: string; protocol?: string; models?: { default?: string }; auth?: { credential_key?: string; env_var?: string } } | undefined;
      const id = (connector?.id ?? f.replace(/\.(yaml|yml)$/, '')) as string;
      if (seen.has(id)) continue;
      const name = (connector?.name ?? parsed['name'] ?? id) as string;
      const credentialKey = (parsed['api_key_vault_key'] ?? connector?.auth?.credential_key) as string | undefined;
      const envVar = (parsed['env_var'] ?? connector?.auth?.env_var) as string | undefined;
      if (!credentialKey && !envVar) continue; // omit connectors with no credential reference
      seen.add(id);
      out.push({
        id,
        name: `${name}  (from llm-gateways/${f})`,
        credentialKey,
        envVar,
...
export function AssessScreen({ onBack, version, scaffold, assessmentType = 'application', onChallenge, onLzChallenge }: AssessScreenProps): JSX.Element | null {
  ...
  // #2262/#2335: component-level licence state for tier-gate enforcement in key
  // handlers and the running-phase line transformer. Mirrors the PublishScreen
  // pattern (lines 149-152). Fail-closed: a load error yields community tier.
  ...
  // #0800: appCursorValue moved to AppPickerPhase local state (#2371).
  ...
  // #0763: appFilter moved to AppPickerPhase local state (#2371).
  ...
  // #2331: LZ target gap details read from lz-catalogue-fit*.yaml after assessment.
  // Keyed by region ID (e.g. "eu-central-1") for matching against lzTargetVerdicts.
  ...
  // #2329/#2351: LLM connector list for the API tokens section of the credential hub.
  ...
  // #2352: paging -- page 0-indexed; page size 9 (one per numeric key per page)
  const [apiTokenPage, setApiTokenPage] = useState(0);
  const API_TOKEN_PAGE_SIZE = 9;
  // #2328: resolved connector + apiKey passed as props into PreflightLlmPhase (#2371).
  ...
  // #2329/#2351: enumerate LLM connectors (built-ins + workspace + llm-gateways/) for the API tokens section.
  // #2352: reset page to 0 on re-entry.
  useEffect(() => {
  ...
  // #2331: read lz-catalogue-fit*.yaml files after a landing-zone assessment
  // to classify each target's BLOCKED reason (sovereignty vs availability).
  // Keyed by the YAML's `region` field so the render can look up by region ID.
  // #2396: lz-catalogue-fit-*.yaml files are at the run-dir root, NOT in passes/.
  useEffect(() => {
    if (phase !== 'assess-done' || assessmentType !== 'landing-zone' || !workspace || !app) return;
    try {
  ...
  // #2328: Route to preflight-llm if the workspace uses an LLM gateway connector,
  // otherwise go directly to 'running'. Reads per-app override at call time.
  const beginRun = () => {
  ...
  // #2328: preflight LLM ping logic moved to PreflightLlmPhase (#2371).
  ...
  useInput((input, key) => {
    ...
    if (phase === 'assess-done' && key.return) {
      if (assessmentType === 'landing-zone' && onLzChallenge && isEnterprise) { setPhase('challenge-prompt'); }
      else if (onChallenge && assessmentType !== 'landing-zone' && isEnterprise) { setPhase('challenge-prompt'); }
      else { onBack(); }
    }
    ...
    // #2262: guard -- challenge module absent on Community/Consultant; pressing C on those tiers crashes.
    ...
    // #0763/#0800: input-app keyboard handlers moved to AppPickerPhase (#2371).
    ...
    // #2329: Esc from API token entry returns to hub
    if (phase === 'input-app-cred-api-token' && key.escape) setPhase('input-app-credentials');
    // #2328: preflight keyboard handlers moved to PreflightLlmPhase (#2371).
    ...
    // #2329/#2352: numeric keys (1-9) for API token connectors; paged
    ...
    // #2352: [ / ] to navigate pages in the API tokens section
    if (phase === 'input-app-credentials' && (input === ']' || key.downArrow)) {
    ...
    if (phase === 'input-app-credentials' && (input === '[' || key.upArrow)) {
    ...
    if (phase === 'input-app-lz-provider' && key.escape) beginRun();
  ...
  // #2333: cap at 95% while still running so the bar never hits 100% before
  // the assess-done phase is reached (the final pass completes then the code
  // transitions; a 100% bar mid-run was misleading to operators).
  const rawDisplayProgress = (phase === 'running' && progressValue === 0)
    ? totalPasses * 0.1
    : progressValue;
  const displayProgressValue = phase === 'running'
    ? Math.min(totalPasses * 0.95, rawDisplayProgress)
    : rawDisplayProgress;

  // ---- render phases ----

  // #2371: input-app phase delegates to shared AppPickerPhase component.
  if (phase === 'input-app') {
    return (
      <AppPickerPhase
        workspace={workspace}
        ...
        onNew={() => setPhase('input-app-new')}
        onNewApp={handleNewAppId}
        onDelete={() => setPhase('pick-delete')}
        onRename={() => setPhase('pick-rename')}
        ...
        <Box marginTop={1}>
          {providerOptions.length > 0 ? (
            <MultiSelect
              ...
              allowEmptyConfirm
              onConfirm={(selected) => {
                if (selected.length === 0) { beginRun(); return; }
                setAppLzCatProviders(selected);
                setAppLzRegionFilter('');
  ...
  if (phase === 'input-app-credentials') {
    ...
    // #2352: paging for API tokens section
    ...
    // #2329: build numeric affordance hint for API token connectors on current page (1-N)
    ...
    const affordances = isLz
      ? [`R -- repo URL  |  T -- access token  |  L -- LLM override${apiNumHint}${apiPageNav}  |  Enter/S -- ${continueLabel}  |  Esc -- back`]
      : [`R -- repo URL  |  A -- access token  |  T -- Playwright URL  |  U -- PW username  |  P -- PW password  |  M -- MFA seed  |  L -- LLM override${apiNumHint}${apiPageNav}  |  Enter/S -- ${continueLabel}  |  Esc -- back`];
    return (
      <Box flexDirection="column" padding={1}>
        <Header subtitle={typeLabel} />
        ...
        {/* #2329/#2351/#2352: API tokens section -- built-in + workspace + llm-gateways/ connectors, paged */}
        <Box marginTop={1} flexDirection="column">
          ...
          {apiPageConnectors.map((c, i) => {
            ...
            const statusText = c.credentialKey
              ? (stored ? '[stored]' : '[  --  ]')
              : '[env-var]';
            const statusColor = stored ? 'green' : 'gray';
            const numLabel = <Text bold={!!c.credentialKey}>{i + 1}</Text>;
            return (
              // #2394: pad badge to fixed 9 chars so numeric shortcut column aligns regardless of badge width
              <Text key={c.id}>    <Text color={statusColor}>{statusText.padEnd(9)}</Text> {numLabel}  {c.name}</Text>
            );
          })}
        </Box>
        <GuidanceBox
          ...
          <TextInput
            ...
            onSubmit={(raw) => {
              ...
              if (val) {
                ...
                // #2324: mirror into vcsUrl state so the token handler can
                // derive the correct provider-scoped credential key.
                setVcsUrl(val);
  ...
  if (phase === 'input-app-cred-vcs-token') {
    // #2324: derive the provider-scoped key from the app's VCS URL so
    // resolveVcsToken() in assess.ts can find it.  Priority: (1) vcsUrl
    // state updated this session, (2) source.vcs.url from .swao.yml, (3)
    // generic fallback.  The old per-app key 'vcs-token-<app>' was never
    // read by the assess pipeline.
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <GuidanceBox
          title="VCS token"
          what="Personal access token or deploy token for cloning the app repository. Stored under the provider-scoped key so the assess pipeline can find it."
          details={[{ label: 'Credential key', value: credKey }]}
          affordances={['Enter -- save and return  |  Enter (empty) -- skip  |  Esc -- back']}
          onOpenChange={(open) => { guidanceOpenRef.current = open; }}
        />
  ...
  // #2329: per-connector API token entry
    ...
    return (
      <Box flexDirection="column" padding={1}>
        ...
        <GuidanceBox
          ...
          what={`API key for the ${connector.name} LLM gateway connector. Stored in the SWAO credential vault under key "${credKey}".`}
          details={[
            { label: 'Connector', value: connector.id },
            { label: 'Credential key', value: credKey },
            { label: 'Model', value: connector.defaultModel || '(not set)' },
          ]}
          affordances={['Enter -- save and return  |  Enter (empty) -- skip  |  Esc -- back']}
          onOpenChange={(open) => { guidanceOpenRef.current = open; }}
          maxRows={Math.max(8, terminalRows - 20)}
  ...
  if (phase === 'confirm-delete') {
    return (
      <Box flexDirection="column" padding={1}>
        ...
        {/* #2323: cap box width to prevent overflow past the header line */}
        <Box flexDirection="column" marginTop={1} borderStyle="single" paddingX={1} width={Math.min(96, (stdout?.columns ?? 80) - 4)}>
        ...
        <GuidanceBox
          title="Delete app"
          what="Permanently removes the app directory and all WSP runs, reports, and exports inside it."
          details={[
            { label: 'Vault credentials', value: 'Vault keys (playwright-url, playwright-pass, playwright-user, playwright-mfa, provider:*:token) are NOT removed. Use swao credential delete to clean them up separately.' },
            { label: 'Action', value: 'y -- confirm delete | n or Escape -- cancel' },
          ]}
          affordances={['y -- confirm delete', 'n or Escape -- cancel']}
        />
      </Box>
    );
  }
  ...
  if (phase === 'op-done') {
    const opWasDelete = opMessage.startsWith('Deleted');
    return (
      <Box flexDirection="column" padding={1}>
        <Header subtitle={typeLabel} />
        ...
        {/* #2323: guidance box on op completion */}
        <GuidanceBox
          title={opWasDelete ? 'Delete complete' : 'Operation complete'}
          what={opWasDelete
            ? 'App directory and all WSP runs removed. Vault credentials for this app were not cleared -- use swao credential delete to remove them.'
            : opError ? 'Operation failed. Check the error above and try again.' : 'Operation completed successfully.'}
          affordances={['Enter or Escape -- return to app list']}
        />
      </Box>
    );
  }
      ...
      <Box flexDirection="column" paddingX={1}>
        ...
        <MultiSelect
          ...
          onConfirm={(selected) => {
            ...
            } else if (assessmentType === 'application') {
              setPhase('input-app-lz-provider');
            } else {
              beginRun();
            }
          }}
  ...
  // #2328: pre-flight LLM connectivity check screen.
  // Shown between the last input step and 'running' when a gateway connector is active.
  // #2371: preflight-llm phase delegates to shared PreflightLlmPhase component.
      ...
      <Box flexDirection="column" padding={1}>
                ...
                <Box flexDirection="column">
                  ...
                  {lzTargetVerdicts.map(tv => {
                    const col = tv.verdict === 'ready' ? 'green'
                      ...
                      : 'red';
                    // #2331: look up gap classification by region ID (last segment after '/')
                    ...
                    return (
                      <Box key={tv.target} flexDirection="column">
                        ...
                        {/* #2331: show partial coverage warning below the verdict line */}
        ...
        <GuidanceBox
          ...
          what={isLz
            ...
            : 'All passes complete. Use Generate Report (4) for text/PDF, or Export BI (6) for the Power BI bundle.'}
          details={[
            { label: 'Run dir', value: `apps/${app}/wsp/runs/<latest>/` },
            // #2331: include sovereignty_statement for the first blocked target (if any)
            ...(() => {
              const blocked = lzTargetVerdicts.find(tv =>
                tv.verdict === 'blocked' || tv.verdict === 'sovereignty_blocked',
              );
              if (!blocked) return [];
          ...
          affordances={[
            ...
            ...(!isLz && onChallenge ? [isEnterprise ? 'C -- Stakeholder Challenge' : 'C -- Stakeholder Challenge [Enterprise only]'] : []),
            ...(isLz && onLzChallenge ? [isEnterprise ? 'C -- LZ Sovereignty Challenge' : 'C -- LZ Sovereignty Challenge [Enterprise only]'] : []),
  ...
  // #0988 Design 074 Step 8 / #2371: post-assessment challenge prompt extracted to shared phase.
  ...
  // #2335: on Community/Consultant the challenge module is absent and the LLM
  // orchestrator emits "challenge complete (0 agents)" progress lines. Replace
  // those with a plain "skipped" label so operators are not confused into
  // thinking challenge ran successfully with zero results.
  const rawDisplayLines = isEnterprise
    ? lines
    : lines.map(l =>
        /\[llm-assessment\].*challenge complete \(0 agents\)/i.test(l)
          ? l.replace(/challenge complete \(0 agents\)/i, 'challenge -- skipped (requires Enterprise tier)')
          : l
      );
  // #2546: LiveOutput wraps long lines via wrap="wrap" -- no per-line truncation needed.
  const displayLines = rawDisplayLines;

  // #2371: active running phase delegates to shared RunningPhase component.
  ...
  return (
    <Box flexDirection="column" padding={1}>
      ...
      <LiveOutput lines={displayLines} maxLines={effectiveLiveLines} label="Output" />
