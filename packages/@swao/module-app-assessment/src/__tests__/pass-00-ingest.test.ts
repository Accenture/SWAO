...
describe('runIngestPrePass (content routing + manifest, #0962 + #0963)', () => {
  ...
  it('warns on image formats, copies to intake/, records extraction:none (#2343)', async () => {
    drop('photo.gif', 'GIF-fake');
    const warnings: string[] = [];
    const manifest = await runIngestPrePass(opts({ warn: (m) => warnings.push(m) }));
    expect(manifest).not.toBeNull();
    // Image lands in manifest.files with extraction marker, not in rejected.
    expect(manifest!.files.some((f) => f.source.includes('photo.gif'))).toBe(true);
    const entry = manifest!.files.find((f) => f.source.includes('photo.gif'));
    expect(entry?.target).toMatch(/^intake\//);
    expect(entry?.extraction).toBe('none');
    expect(entry?.warning).toBe('image-no-text-extraction');
    expect(warnings.some((w) => w.includes('photo.gif') && w.includes('image format'))).toBe(true);
    expect(existsSync(join(ws, 'wsp', 'inputs', 'intake', 'photo.gif'))).toBe(true);
  });
