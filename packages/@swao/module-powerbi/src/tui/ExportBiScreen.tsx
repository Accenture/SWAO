...
import { HeaderView, type LicenseStateView, TextInput, SelectInput, MultiSelect, LiveOutput, GuidanceBox, LicenseGate, isAllowed, ProgressBar } from '@swao/tui-kit';
import { findWorkspace, LicenseGuard, logApp, setWorkspaceRoot } from '@swao/core';
...
const TARGET_OPTIONS = [
  ...
  { label: 'Star schema for Tableau  -- [coming soon]',                                      value: 'tableau' },
];
...
export function ExportBiScreen({ onBack, version }: ExportBiScreenProps) {
  ...
  // Licence state for Enterprise-gated affordances (done-summary continue prompt,
  // done-powerbi page). Defaults to community on any load error so the screen
  // remains usable for lower tiers.
  const licenseState: LicenseStateView = useMemo(() => {
    try { return LicenseGuard.load().state as LicenseStateView; }
    catch { return { tier: 'community', assessmentCount: 0, firstRun: new Date().toISOString().slice(0, 10) }; }
  }, []);
  ...
  // #0735: auto-skip target picker when only one active (non-coming-soon) target exists.
  useEffect(() => {
    if (phase !== 'input-target') return;
    const active = TARGET_OPTIONS.filter(o => !o.label.includes('[coming soon]'));
    if (active.length === 1 && active[0]) handleTargetSelect(active[0].value);
  ...
  useEffect(() => {
    ...
    if (workspace) setWorkspaceRoot(workspace);
    ...
    const push = (chunk: Buffer) => {
      ...
      for (const line of newLines) {
        try { logApp(app, 'info', 'tui.output', line); } catch { /* best-effort */ }
      }
  ...
  useInput((input, key) => {
    ...
    if (phase === 'done-summary' && key.return && isAllowed(licenseState, 'enterprise')) setPhase('done-powerbi');
  ...
  const handleTargetSelect = (v: string) => {
    setTarget(v);
    if (v === 'tableau') {
      setNotice('Tableau template export is not yet available. Use PowerBI for now.');
      setPhase('running');
    } else {
      setNotice('');
    ...
    <Box flexDirection="column" padding={1}>
      ...
      {phase === 'running' && (
        <>
          ...
          {/* #2582: indeterminate progress bar while BI bundle is being emitted */}
          <ProgressBar value={lines.length > 0 ? Math.min(lines.length, 19) : 0.5} total={20} label="Emitting BI bundle..." color="cyan" width={28} />
          <Box marginTop={1}><Text dimColor>--- Output ---</Text></Box>
          ...
          <GuidanceBox
            ...
            title="Export BI"
            what="Emitting the BI star-schema bundle. Files are written to apps/<id>/wsp/exports/<timestamp>/ when complete."
            onOpenChange={(open) => { guidanceOpenRef.current = open; }}
          />
        </>
      )}
      ...
      {phase === 'done-summary' && (
        <>
          ...
          <Box marginTop={1} flexDirection="column">
            {/* #2415: show workspace-relative paths to avoid long absolute paths overflowing the terminal */}
            <Text>  Star schema: <Text color="cyanBright">{bundleDir ? (workspace ? relative(workspace, bundleDir) : bundleDir) : '(see live output for path)'}</Text></Text>
            {bundleDir ? (
              <Text>  XLSX rollup: <Text color="cyanBright">{workspace ? relative(workspace, bundleDir.replace(/[/\\]star$/, '') + (process.platform === 'win32' ? '\\' : '/') + 'xlsx' + (process.platform === 'win32' ? '\\' : '/') + 'swao-export.xlsx') : bundleDir.replace(/[/\\]star$/, '') + (process.platform === 'win32' ? '\\' : '/') + 'xlsx' + (process.platform === 'win32' ? '\\' : '/') + 'swao-export.xlsx'}</Text></Text>
            ) : null}
            <Text>  App:         <Text color="cyanBright">{app}</Text></Text>
            <Text>  Files:       <Text color="cyanBright">CSV star schema ({csvCount > 0 ? csvCount : 22} files) + NDJSON mirror + XLSX rollup</Text></Text>
            {/* #2414: pbit templates are Enterprise-only; show tier marker in Community/Consultant */}
            <Text>  Templates:   {isAllowed(licenseState, 'enterprise')
              ? <Text color="cyanBright">swao-report.pbit{portfolioPbitPath ? ' + swao-portfolio.pbit' : ''}</Text>
              : <Text dimColor>[Enterprise] swao-report.pbit + swao-portfolio.pbit</Text>
            }</Text>
          </Box>
          {isAllowed(licenseState, 'enterprise') && (
            <Box marginTop={1} flexDirection="column" paddingX={1} width={Math.min(98, Math.max(61, (stdout?.columns ?? 80) - 2))}>
      ...
      {phase === 'done-powerbi' && !isAllowed(licenseState, 'enterprise') && (
        <LicenseGate
          required="enterprise"
          state={licenseState}
          feature="PowerBI artefact links (template paths, hot-keys, SWAOWorkspaceRoot copy)"
          onOpenLicenseScreen={onBack}
          onBack={onBack}
        >
          <></>
        </LicenseGate>
