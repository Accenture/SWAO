...
import { LicenseGuard, LicenseTierError, LicenseLimitError, LicenseInvalidError, logPortfolio, setWorkspaceRoot } from '@swao/core';
...
export function registerExport(program: Command): void {
  program
    ...
    .action(async (opts: ExportOptions) => {
      ...
      // #2246: ensure log events land in the workspace regardless of cwd.
      setWorkspaceRoot(workspaceRoot);
      const exportStartMs = Date.now();
      logPortfolio('info', 'export.start', `BI export starting${opts.app ? ` -- app: ${opts.app}` : ' (portfolio)'}`, {
        context: { app: opts.app ?? null, mode: opts.portfolio ? 'portfolio' : 'single', formats: opts.formats ?? 'csv,ndjson' },
      });
      ...
      if (opts.portfolio) {
        try {
          ...
          if (err instanceof LicenseTierError || err instanceof LicenseLimitError) {
            try { logPortfolio('error', 'licence.gate.error', 'swao export --portfolio requires an Enterprise licence', { context: { feature: 'export --portfolio', required: 'enterprise', code: 1 } }); } catch { /* best-effort */ }
            console.error([
              '[LICENSE] swao export --portfolio requires an Enterprise license.',
              'Run `swao license request --tier enterprise` to obtain a license.',
            ].join('\n'));
            process.exit(1);
          }
          if (err instanceof LicenseInvalidError) {
            try { logPortfolio('error', 'licence.gate.error', `Invalid licence: ${(err as LicenseInvalidError).code}`, { context: { feature: 'export --portfolio', code: 3, reason: (err as LicenseInvalidError).code } }); } catch { /* best-effort */ }
        ...
        logPortfolio('info', 'export.complete', `Portfolio BI export complete -- ${r.apps.length} app(s)`, {
          context: { mode: 'portfolio', apps: r.apps.length, bundle_dir: r.bundleDir, duration_ms: Date.now() - exportStartMs },
        });
        return;
      }

      if (!opts.app) {
        logPortfolio('error', 'export.error', '--app required for single-app export');
      ...
      if (!existsSync(workspaceAppDir)) {
        logPortfolio('error', 'export.error', `App '${opts.app}' not found in workspace`, { context: { app: opts.app } });
        console.error(`[error] App '${opts.app}' not found in workspace.\n  Expected: ${workspaceAppDir}`);
        process.exit(1);
      }
      if (!existsSync(join(workspaceAppDir, 'wsp'))) {
        logPortfolio('error', 'export.error', `No wsp/ under ${workspaceAppDir}`, { context: { app: opts.app } });
      ...
      if (bundleDir === null) {
        logPortfolio('error', 'export.error', 'No formats selected', { context: { app: opts.app } });
        console.error(`[error] No formats selected. Use --formats csv,ndjson,xlsx (any subset).`);
        process.exit(1);
      }

      // Total file count across all selected formats for the completion event.
      const totalFiles =
        (csvResult?.manifest.files.length ?? 0) +
        (ndjsonResult?.manifest.files.length ?? 0) +
        (xlsxResult?.manifest.files.length ?? 0);
      logPortfolio('info', 'export.complete', `BI export complete -- app: ${opts.app}`, {
        context: { app: opts.app, bundle_dir: bundleDir, file_count: totalFiles, duration_ms: Date.now() - exportStartMs },
      });
