...
function buildStarTables(ctx: ExportContext): StarTables {
  ...
  for (const { dir, prefix } of lzSearchDirs) {
    ...
    for (const lzf of lzFiles) {
      const lzRaw = safeReadYaml<Record<string, unknown>>(join(dir, lzf));
      if (!lzRaw) continue;
      // #2557: Two file structures exist:
      // - passes/lz-fit.yaml  --> data wrapped under 'assessment' key with sibling 'pass' key
      // - run-root lz-catalogue-fit-*.yaml --> flat wsp['lz'] dump (no 'assessment' wrapper)
      const a = (lzRaw['assessment'] as Record<string, unknown> | undefined) ?? lzRaw;
      const passData = (lzRaw['pass'] as Record<string, unknown> | undefined) ?? {};
      const provider = String(a['provider'] ?? '');
      const region = String(a['region'] ?? '');
      if (!provider && !region) continue;
      const lzId = `${provider}/${region}`;
      const verdict = String(a['overall'] ?? '');
      const assessedAt = String(passData['assessed_at'] ?? a['generated_at'] ?? '');
      const notMetItems = ((a['items'] as Array<Record<string, unknown>>) ?? []).filter((it) => it['status'] === 'NOT_MET');
      const gapCount = notMetItems.length;
      factLzRows.push([lzId, appId, provider, region, verdict, gapCount, String(a['assessment_mode'] ?? ''), assessedAt]);
      ...
      for (const item of notMetItems) {
        const gapCode = `${String(item['framework'] ?? '')}/${String(item['requirement'] ?? '')}`;
...
export function writePortfolioStarExport(ctx: PortfolioExportContext): WritePortfolioStarResult {
  ...
  writeFileSync(
    ...
    portfolioExportHeader + [
      ...
      '  set `SWAOWorkspaceRoot` to the workspace root; the template computes the star/ path automatically.',
