...
export const CONNECTOR_PROTOCOLS = ['openai-chat', 'anthropic-messages', 'ollama', 'bedrock'] as const;
...
const MetaSchema = z.object({
  source: z.enum(['bundled', 'user', 'workspace']).optional(),
  last_reviewed: z.string().optional(),
  contributor: z.string().optional(),
  fetched_at: z.string().optional(),
}).strict();

// #2748: enterprise PKI support -- connector-level TLS override.
// ca_bundle: filesystem path to a PEM-format CA bundle; injected into the
// process-level TLS context so pkg-bundled Node.js trusts internal CAs
// without requiring NODE_EXTRA_CA_CERTS to be set in the environment.
// reject_unauthorized: false disables cert verification (corporate MITM
// proxies that cannot provide a CA bundle); emits a [warn] at probe time.
...
export type ConnectorTls = z.infer<typeof TlsSchema>;
