...
// SSE streaming helper -- AnthropicLlmProvider reads response.body as SSE (#1767).
function makeSseResponse(text: string, inputTokens = 10, outputTokens = 20): Partial<Response> {
  const enc = new TextEncoder();
  const events = [
    { type: 'message_start', message: { usage: { input_tokens: inputTokens } } },
    { type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text } },
    { type: 'message_delta', usage: { output_tokens: outputTokens }, delta: { stop_reason: 'end_turn' } },
  ];
...
describe('AnthropicLlmProvider (#0117)', () => {
  ...
  it('calls the Anthropic messages endpoint and returns text block', async () => {
    const mockFetch = vi.fn().mockResolvedValue(makeSseResponse('anthropic output'));
    vi.stubGlobal('fetch', mockFetch);
