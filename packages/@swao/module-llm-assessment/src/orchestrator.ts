...
export async function orchestrateLegs(deps: OrchestratorDeps): Promise<OrchestrationResult> {
  ...
  // #2284: emit swao.llm.run.start so the log monitor can distinguish LLM
  // orchestration events from provider-transport events (provider.llm.*).
  logApp(deps.appId, 'info', 'swao.llm.run.start', `LLM Assessment starting: ${deps.legs.length} legs`, {
    context: { legs: deps.legs.length, execution: deps.execution, source_hash: sourceHash },
  });
  ...
  const runOneLeg = async (leg: ResolvedLeg): Promise<void> => {
    ...
    let legWorkspace = '';
    // #2516: must be outside try so the finally block can read it.
    ...
    try {
      ...
      heartbeatTimer = setInterval(() => {
        const elapsed_ms = Date.now() - legStartMs;
        let calls_so_far = 0;
        try {
          if (existsSync(sink)) {
            const raw = readFileSync(sink, 'utf-8');
            calls_so_far = raw.split('\n').filter((l) => l.trim()).length;
          }
        } catch { /* non-fatal */ }
        log.write('info', 'leg.heartbeat', `leg ${leg.id} in progress`, { leg_id: leg.id, elapsed_ms, calls_so_far });
        logApp(deps.appId, 'info', 'leg.heartbeat', `leg ${leg.id} in progress`, { context: { leg_id: leg.id, elapsed_ms, calls_so_far } });
      }, 30_000);
      ...
      // #2287: mirror leg.complete to app-events so the sequence gate sees the closure.
      logApp(deps.appId, 'info', 'leg.complete', `leg ${leg.id} complete`, {
        context: { leg_id: leg.id, calls: legRecords.length, wall_clock_ms: result.durationMs },
      });
      ...
      if (deps.spawnChallenge) {
        ...
        try {
          ...
          // #2290: add leg_id to challenge complete events for log correlation.
          log.write('info', 'leg.challenge.complete', `leg ${leg.id} challenge complete`, {
            leg_id: leg.id, agents: challengeResult.agents.length, exit_code: challengeResult.exit_code,
          });
      ...
      if (deps.spawnLzChallenge) {
        ...
        try {
          ...
          log.write('info', 'leg.challenge-lz.complete', `leg ${leg.id} LZ challenge complete`, {
            leg_id: leg.id, agents: lzChallengeResult.agents.length, exit_code: lzChallengeResult.exit_code,
          });
          progress(`leg ${leg.id}: LZ challenge complete (${lzChallengeResult.agents.length} agents)`);
        } catch (lzChallengeErr) {
          log.write('warn', 'leg.challenge-lz.error', `leg ${leg.id} LZ challenge failed: ${(lzChallengeErr as Error).message}`, { leg_id: leg.id });
      ...
      // #2427: unhandled leg throw (spawnLeg rejection, cloneWorkspace failure, etc.)
      // previously propagated out of runOneLeg, causing Promise.all to reject before
      // manifest.yaml and findings.yaml could be written. Convert to a recorded finding
      // so the orchestration continues and always writes final artefacts.
      const errMsg = legErr instanceof Error ? legErr.message : String(legErr);
      log.write('error', 'leg.error', `leg ${leg.id} threw: ${errMsg}`, { leg_id: leg.id });
      logApp(deps.appId, 'error', 'leg.error', `leg ${leg.id} threw: ${errMsg}`, { context: { leg_id: leg.id } });
      findings.add({
        severity: 'error', leg: leg.id, type: 'leg-failed',
        message: `leg ${leg.id} threw an unhandled error: ${errMsg}`,
      });
    } finally {
      ...
      if (legWorkspace) {
        if (deps.keepLegWsp) {
          ...
          // #2516: stall watchdog fired -- skip rmSync; the subprocess is still
          // running and holds file handles on Windows. Let the OS temp-dir sweep
          // clean up the workspace after the process eventually exits.
          log.write('warn', 'leg.workspace-cleanup-skipped',
            `leg ${leg.id} stall: workspace cleanup deferred (process may still be running)`, { path: legWorkspace });
        } else {
          // #2574: on Windows a watchdog-killed child process may still hold file
          // handles briefly after the exit event. EBUSY/EPERM on rmSync is transient;
          // swallow it and let the OS temp-dir sweep clean up later.
          try {
            ...
            log.write('warn', 'leg.workspace-cleanup-skipped',
              `Cannot delete leg workspace (${code}): left for OS cleanup`, { path: legWorkspace });
          }
        }
      }
    }
  ...
  // #2525: wrap entire synthesis phase so run.complete is always emitted even
  // if assembleGroups / finalResult / writeFileSync throws unexpectedly.
  try {
  ...
  // #2407: include run cost/duration metadata block for operator reporting.
  const runDurationMs = Date.now() - runStartMs;
  const runBlock: Record<string, unknown> = {
    run_ts: ts,
    duration_minutes: Math.round(runDurationMs / 6000) / 10,
    legs: Object.fromEntries(deps.legs.map((leg) => {
      const legRecords = allRecords.filter((r) => r.leg.id === leg.id);
      const costUsd = legRecords.reduce((acc, r) => acc + (r.cost_usd.computed ?? 0), 0);
      const wallMs = legStats[leg.id]?.wallClockMs ?? 0;
      return [leg.id, {
        cost_usd: Math.round(costUsd * 1_000_000) / 1_000_000,
        duration_minutes: Math.round(wallMs / 6000) / 10,
        call_count: legRecords.length,
      }];
    })),
  };
  ...
  // #2293: add structured context fields so tooling can extract call_count/leg_count without
  // parsing the human-readable message string.
  // #2284: emit swao.llm.run.complete so the log monitor closes the swao.llm.run.start
  // sequence and can confirm the LLM assessment reached a normal completion.
  logApp(deps.appId, 'info', 'swao.llm.run.complete', `LLM Assessment complete: ${allRecords.length} calls across ${deps.legs.length} legs`, {
    context: { call_count: allRecords.length, leg_count: deps.legs.length, duration_ms: Date.now() - runStartMs, findings: findings.all().length },
  });
  log.write('info', 'run.complete', `LLM Assessment complete: ${allRecords.length} calls across ${deps.legs.length} legs`, {
    ...
    call_count: allRecords.length,
    leg_count: deps.legs.length,
  ...
  return { runDir, manifestPath, records: allRecords, groups, final, findingsCount: findings.all().length };
    ...
    log.write('error', 'run.complete', `LLM Assessment synthesis failed: ${errMsg}`, {
      run_dir: runDir, call_count: allRecords.length, leg_count: deps.legs.length, duration_ms: Date.now() - runStartMs,
    });
    logApp(deps.appId, 'error', 'swao.llm.run.complete', `LLM Assessment synthesis failed: ${errMsg}`, {
      context: { error: errMsg, call_count: allRecords.length, leg_count: deps.legs.length, duration_ms: Date.now() - runStartMs },
    });
    throw synthErr;
  }
}
