...
export interface RunLogEvent {
  ...
  // #2387: renamed from 'event' to 'code' to match the field name read by
  // LlmAssessmentScreen and the UAT monitors (all readers use .code, not .event).
  code: string;            // dotted: leg.start, call.complete, ...
  message: string;
...
export class RunLog {
  ...
  write(level: RunLogEvent['level'], code: string, message: string, context?: Record<string, unknown>): void {
    const entry: RunLogEvent = { ts: this.clock(), level, code, message, ...(context ? { context } : {}) };
