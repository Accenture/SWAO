...
interface ModeEntry {
  ...
  output: string;
  tier: string;
  /** If set, user must have this tier or higher to activate the mode. */
  tierGate?: Exclude<LicenseTier, 'community'>;
  comingSoon?: boolean;
  advanced?: boolean;
}
...
export function PublishScreen({ onBack, version }: PublishScreenProps) {
  ...
  // Load licence state for tier-gate enforcement in mode-select (#2308).
  ...
  // #2658: warn when challenge output is absent so user knows to run challenge first.
  ...
  useInput((input, key) => {
    ...
    if (phase === 'mode-select') {
      ...
      if (key.return || found) {
        if (!selected) return;
        // Enforce tier gate before navigating (#2308)
        if (selected.tierGate && !isAllowed(licenseState, selected.tierGate)) {
          setComingSoonMsg(
            `HTML Editor requires an Enterprise licence. Run 'swao license request' to upgrade.`,
          );
          return;
        }
  ...
  useEffect(() => {
    ...
    // #2658/#2681: check for challenge output (ts-dir preferred; combined.yaml fallback).
    if (workspace && appId) {
      const challengeDir = join(workspace, 'apps', appId, 'wsp', 'challenge-app');
      let hasChallengeData = false;
      if (existsSync(challengeDir)) {
        try {
          const entries = readdirSync(challengeDir, { withFileTypes: true });
          const hasTsDir = entries.some(e => e.isDirectory() && /^\d{4}-\d{2}-\d{2}T/.test(e.name));
          const hasCombined = entries.some(e => e.isFile() && e.name === 'combined.yaml');
          hasChallengeData = hasTsDir || hasCombined;
        } catch { /* ignore FS errors */ }
      }
      if (!hasChallengeData) {
        setChallengeWarning(
          'No stakeholder challenge data found. Run `swao challenge --all-agents` before publishing to include the Stakeholder Challenge section.',
        );
      }
    }
    ...
    <Box flexDirection="column">
      ...
      {phase === 'mode-select' && (
        <Box flexDirection="column" paddingX={1}>
          {ACTIVE_MODES.map((m, activeIdx) => {
            ...
            const tierBlocked = !!m.tierGate && !isAllowed(licenseState, m.tierGate);
            const tierBadge = tierBlocked
              ? (m.tierGate === 'enterprise' ? '  [Enterprise]' : '  [Consultant+]')
              : '';
            return (
              <Box key={m.mode}>
                <Text color={active && !tierBlocked ? 'cyan' : undefined}>{active ? '> ' : '  '}</Text>
                <Text color="yellow" dimColor={tierBlocked}>[{m.key}]</Text>
                <Text> </Text>
                <Text bold={!tierBlocked} color={active && !tierBlocked ? 'cyan' : undefined} dimColor={tierBlocked}>
                  {m.label}{tierBadge}
                </Text>
      ...
      {phase === 'input-app' && (
        <Box flexDirection="column" paddingX={2}>
          ...
          <GuidanceBox
            ...
            onOpenChange={(open) => { guidanceOpenRef.current = open; }}
          />
          <Text color="gray">Enter: confirm | Esc: back to mode select</Text>
        </Box>
      )}
      ...
      {phase === 'editor-confirm' && (
        <Box flexDirection="column" paddingX={2} marginTop={1}>
          ...
          <GuidanceBox
            title="HTML Editor"
            what="Opens the SWAO Publication Editor at localhost:4001. Customise layout, block order, and branding in your browser, then export the final HTML."
            details={[
              { label: 'Layout tab',  value: 'Toggle top-nav links and reorder sidebar items' },
              { label: 'Style tab',   value: 'Adjust brand colours and visual presets' },
            ]}
            affordances={['Enter -- launch editor  |  Esc -- back to mode picker']}
            onOpenChange={(open) => { guidanceOpenRef.current = open; }}
          />
        </Box>
      )}
      ...
      {phase === 'running' && mode !== 'editor' && (
        <Box flexDirection="column" paddingX={2}>
          ...
          {challengeWarning && (
            <Box marginTop={1} borderStyle="single" borderColor="yellow" paddingX={1}>
              <Text color="yellow">[warn] {challengeWarning}</Text>
            </Box>
          )}
          <Box marginTop={1}>
            <GuidanceBox
      ...
      {phase === 'done' && (
        <Box flexDirection="column" paddingX={2}>
          ...
          {outputPath && (
            <Box flexDirection="column" marginTop={1}>
              ...
              <Box marginTop={1}>
                ...
                <Text color="gray">  (also: letter O)</Text>
          ...
          <Text color="gray">Enter/O(letter): open in browser  |  Esc: back</Text>
        </Box>
      )}
      ...
      {phase === 'error' && (
        <Box flexDirection="column" paddingX={2}>
          ...
          <Box marginTop={1}>
            {/* #2275 #2310: differentiate guidance by error type so licence failures
                do not mislead the user into running another assessment. */}
            {errorMsg.includes('[LICENSE]') ? (
              <GuidanceBox
                title="Licence Required"
                what="This feature requires a higher licence tier. Open the Licence screen (Tools (9) -> Licence) and run 'Request a licence' to upgrade."
