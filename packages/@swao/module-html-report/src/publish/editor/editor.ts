...
export async function launchEditor(opts: { port?: number; appId?: string; profileVariant?: string }): Promise<void> {
  ...
  // #2640: emit publish.editor.start so operators can audit when an editor session began.
  try { logPortfolio('info', 'publish.editor.start', `Publication editor started at ${url}`, { context: { port: boundPort, mode: 'editor' } }); } catch { /* best-effort */ }
  ...
  await new Promise<void>((resolve) => {
    process.once('SIGINT', () => {
      // #2640: emit publish.editor.stop before shutting down the server.
      try { logPortfolio('info', 'publish.editor.stop', 'Publication editor stopped'); } catch { /* best-effort */ }
