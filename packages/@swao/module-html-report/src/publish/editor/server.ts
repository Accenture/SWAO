...
import { findWorkspace, logPortfolio } from '@swao/core';
...
async function handleExportLevel1(
  ...
  try {
    ...
    // #2640: emit publish.template.export so operators can audit when a custom template was saved.
    try {
      logPortfolio('info', 'publish.template.export', `Template exported (Level 1): ${tmplPath}`, {
        context: { path: tmplPath, level: 1, profile: safeProfile, block_order: body.blocksOrder ?? [] },
      });
    } catch { /* best-effort */ }
...
<script type="application/json" id="swao-pub-data">/* injected by renderer */</script>
    ...
    // #2640: emit publish.template.export for Level 2 shell export.
    try { logPortfolio('info', 'publish.template.export', `Template exported (Level 2): ${stubPath}`, { context: { path: stubPath, level: 2 } }); } catch { /* best-effort */ }
