...
export class CredentialStore {
  ...
  private load(): { data: Record<string, string>; salt?: Buffer } {
    ...
    try {
      ...
      logPortfolio('error', 'credential.store.error', 'Credential vault read failed', {
  ...
  async set(name: string, value: string): Promise<void> {
    ...
    data[name] = value;
    this.save(data, salt);
    // #2325: audit log -- key name only, never the value
    logPortfolio('info', 'credential.stored', 'Credential stored', { context: { key: name } });
  }
  ...
  async delete(name: string): Promise<boolean> {
    ...
    this.save(data, salt);
    // #2325: audit log -- key name only, never the value
    logPortfolio('info', 'credential.deleted', 'Credential deleted', { context: { key: name } });
    return true;
  }
  ...
  // #2325 Feature 2: import a Record of credentials in merge or replace mode.
  // Merge: existing keys not in data are preserved. Replace: vault is cleared.
  // Logs credential.stored for each imported key (key name only, never value).
  async importAll(data: Record<string, string>, mode: 'merge' | 'replace'): Promise<void> {
    const { data: existing, salt } = mode === 'merge' ? this.load() : { data: {} as Record<string, string>, salt: undefined };
    const merged = { ...existing, ...data };
    this.save(merged, salt);
    for (const key of Object.keys(data)) {
      logPortfolio('info', 'credential.stored', 'Credential stored', { context: { key } });
    }
  }
