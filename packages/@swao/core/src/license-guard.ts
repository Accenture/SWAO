...
// #2588: resolve a user-scoped, machine-global data dir for the licence file.
// No admin rights required; survives workspace deletion; one seat per machine.
function resolveAppDataDir(): string {
  const p = platform();
  if (p === 'win32') {
    // %LOCALAPPDATA%\swao  (e.g. C:\Users\<user>\AppData\Local\swao)
    return join(process.env['LOCALAPPDATA'] ?? join(homedir(), 'AppData', 'Local'), 'swao');
  }
  if (p === 'darwin') {
    return join(homedir(), 'Library', 'Application Support', 'swao');
  }
  // Linux / other POSIX: ${XDG_CONFIG_HOME:-~/.config}/swao
  return join(process.env['XDG_CONFIG_HOME'] ?? join(homedir(), '.config'), 'swao');
}

// Exported so tests can override paths without mocking node:os
export const _paths = {
  statePath: join(homedir(), '.swao-state.json'),
  licensePath: join(resolveAppDataDir(), 'licence.json'),
  // Old per-user home path used before #2588; kept for one-time migration.
  // Exposed so tests can point it at a non-existent path to skip migration.
  legacyLicencePath: join(homedir(), '.swao-license.json'),
};
...
export interface LicenseState {
  ...
  exp?: string;
  /** ISO-8601 date the licence was issued (from payload `iat`). Used to
   *  calculate the elapsed fraction of the licence's validity window in the
   *  TUI status panel -- fixes the progress bar that previously used `firstRun`
   *  (machine first-use date) as the period start. Absent for Community. */
  issuedAt?: string;
  licensee?: string;
  email?: string;
  ...
  organisation?: string;
  /** When set, a paid licence expired and the session was silently
   *  downgraded to Community. The value is the original tier (e.g.
   *  'enterprise'). Used by the TUI to show an expiry banner. */
  expiredTier?: LicenseTier;
}
...
// #2633: layered hardware-ID fallback chain (ADR-0051).
// Returns the most stable hardware identifier available on the current platform,
// or an empty string when no hardware source is accessible (containers/sandboxes).
// Uses only spawnSync (not async) because computeFingerprint is called
// from readStateFile which is synchronous. wmic/ioreg are direct commands
// and do not spawn grandchildren, so spawnSync timeout is a reliable deadline.
function readHardwareId(): string {
  const p = platform();
  try {
    if (p === 'win32') {
      const r = spawnSync('wmic', ['csproduct', 'get', 'UUID', '/value'], {
        encoding: 'utf-8', timeout: 2000, windowsHide: true,
      });
      if (r.status === 0 && r.stdout) {
        const m = /UUID=([A-F0-9]{8}-[A-F0-9]{4}-[A-F0-9]{4}-[A-F0-9]{4}-[A-F0-9]{12})/i.exec(r.stdout);
        if (m) return m[1]!;
      }
    } else if (p === 'darwin') {
      ...
      if (r.status === 0 && r.stdout) {
        const m = /"IOPlatformUUID"\s*=\s*"([^"]+)"/i.exec(r.stdout);
        if (m) return m[1]!;
      }
    } else {
      // Linux: try DMI product_uuid (requires root on some distros but readable in most VMs)
      const dmi = '/sys/class/dmi/id/product_uuid';
      if (existsSync(dmi)) {
        const id = readFileSync(dmi, 'utf-8').trim();
        if (id) return id;
      }
    }
  } catch { /* fallthrough to next level */ }

  // Level 2: /etc/machine-id (Linux, WSL, some containers)
  try {
    const machineId = '/etc/machine-id';
    if (existsSync(machineId)) {
      const id = readFileSync(machineId, 'utf-8').trim();
      if (id) return id;
    }
  } catch { /* fallthrough */ }

  // Level 3: primary NIC MAC address (cross-platform; skips loopback + virtual)
  try {
  ...
  // Level 4: no hardware source available (pure-container / unit-test sandbox)
  return '';
}

function computeFingerprint(firstRun: string): string {
  // #2633 (ADR-0051): fp_v2 includes a hardware component so the fingerprint
  // cannot be transferred to another machine by copying the state file alone.
  // Derivation: SHA256('fp_v2|' + hostname + platform + firstRun + hwId)[0..16]
  const hwId = readHardwareId();
  return createHash('sha256')
    .update('fp_v2|' + hostname() + platform() + firstRun + hwId)
    .digest('hex')
    .substring(0, 16);
}
...
function readStateFile(): StateFile {
  if (!existsSync(_paths.statePath)) {
    ...
    const state: StateFile = { first_run: firstRun, assessment_count: 0, fingerprint, fp_version: 2 };
    writeFileSync(_paths.statePath, JSON.stringify(state, null, 2), 'utf-8');
    return state;
  }
  const state = JSON.parse(readFileSync(_paths.statePath, 'utf-8')) as StateFile;
  // #2633: migrate existing state files to the hardware-bound fp_v2 fingerprint
  // on first run of a binary that includes ADR-0051. NODE_ENV === 'test' skips
  // the rewrite so test fixtures (which set their own fingerprint) are unaffected.
  if (!state.fp_version && process.env['NODE_ENV'] !== 'test') {
    state.fingerprint = computeFingerprint(state.first_run);
    state.fp_version = 2;
    writeFileSync(_paths.statePath, JSON.stringify(state, null, 2), 'utf-8');
  }
  return state;
...
function verifyKey(rawKey: string, localFp: string): LicensePayload {
  ...
  // #2652/#2665: accept exact match (16-char) or legacy prefix match (8-char)
  // so keys issued before the sprint-132 fp-length fix continue to work.
  if (localFp !== payload.fp && !localFp.startsWith(payload.fp)) {
    throw new LicenseInvalidError(
      [
        'This license key was issued for a different machine.',
        `Your machine fingerprint: ${localFp}`,
...
export class LicenseGuard {
  ...
  static load(): LicenseGuard {
    ...
    // One-time migration: if the new path does not yet exist but the legacy
    // homedir path does, copy it forward so the operator keeps their tier
    // without needing to re-activate (#2588).
    if (!existsSync(_paths.licensePath) && existsSync(_paths.legacyLicencePath)) {
      try {
        mkdirSync(dirname(_paths.licensePath), { recursive: true });
        copyFileSync(_paths.legacyLicencePath, _paths.licensePath);
      } catch { /* non-fatal: will fall through to Community */ }
    }
    ...
    if (existsSync(_paths.licensePath)) {
      ...
      if (isExpired) {
        ...
        // #2632: set expiredTier so the TUI can show a notification banner.
        return new LicenseGuard({
          ...
          email: payload.email,
          expiredTier: normalizeTier(payload.tier),
        });
      }
