...
export function renderCoverageBar(model: PublicationModel, donutOpts?: Record<string, string>, sevBarOpts?: Record<string, string>): string {
  ...
  const coverageDonut = renderChartDonut(pct, score, { ...donutOpts, color: '#f97316', label: 'Assessment coverage' });
  const sevBarHtml = renderChartSeverityBar(counts as Record<string, number>, sevBarOpts);

  // #2675: cloud-native and portability score tiles (optional -- absent on older WSP).
  const cnScore = (model.summary as Record<string, unknown>).cloud_native_score as number | undefined;
  const ptScore = (model.summary as Record<string, unknown>).portability_score as number | undefined;

  const cnTile = cnScore != null ? (() => {
    const cnPct = Math.round(cnScore * 100);
    const cnDonut = renderChartDonut(cnPct, cnScore, { size: donutOpts?.size ?? 'medium', color: '#2563eb', label: 'Cloud-native score' });
    return `<div class="coverage-widget">
    ${cnDonut}
    <div class="coverage-details">
      <div class="coverage-percent">${cnPct}%</div>
      <div class="coverage-label" data-i18n-key="block.coverage_bar.cloud_native_label">Cloud-Native Score</div>
      <div class="coverage-subtext" data-i18n-key="block.coverage_bar.cloud_native_desc">Cloud-native patterns observed (12-factor, containers, stateless design). Low = significant lift required.</div>
    </div>
  </div>`;
  })() : '';

  const ptTile = ptScore != null ? (() => {
    const ptPct = Math.round(ptScore * 100);
    const ptDonut = renderChartDonut(ptPct, ptScore, { size: donutOpts?.size ?? 'medium', color: '#16a34a', label: 'Portability score' });
    return `<div class="coverage-widget">
    ${ptDonut}
    <div class="coverage-details">
      <div class="coverage-percent">${ptPct}%</div>
      <div class="coverage-label" data-i18n-key="block.coverage_bar.portability_label">Portability Score</div>
      <div class="coverage-subtext" data-i18n-key="block.coverage_bar.portability_desc">Estimated workload portability across cloud providers. Low = strong vendor lock-in.</div>
    </div>
  </div>`;
  })() : '';

  // #2702: migration rationale paragraph from synth pass
  const rationale = (model.summary as Record<string, unknown>).migration_rationale as string | undefined;
  const rationaleHtml = rationale
    ? `<p class="migration-rationale" data-i18n-key="block.coverage_bar.migration_rationale">${esc(rationale)}</p>`
    : '';
