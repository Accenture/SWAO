...
export function renderChartDonut(pct: number, score: number, opts?: Record<string, string>): string {
  ...
  const colour = opts?.color ?? '#f97316';
  const titleLabel = opts?.label ?? 'Assessment coverage';
  return `<div class="donut-container" aria-hidden="true">
      <svg width="${dim}" height="${dim}" viewBox="0 0 ${dim} ${dim}" aria-label="${titleLabel} ${pct}%" role="img">
        <title>${titleLabel}: ${pct}%</title>
