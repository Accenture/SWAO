...
export function renderLzrSummary(model: PublicationModel): string {
  ...
  if (regions && regions.length > 1) {
    ...
    // #2311: include actionable CTA in the sovereignty-inactive callout.
    ...
    <div class="pub-mt-2"><strong>Next action:</strong> Add <code>assessment.regimes_active: [GDPR]</code>
    (or your applicable frameworks) to your <code>.swao.yml</code>, then re-run <code>swao assess</code>
    to generate a verified sovereignty verdict.</div>
  </div>`
      : '';
    ...
    <tr class="pub-tr-border ${rowModClass}">
      ...
      <td class="pub-td-mid-right">
        ${r.overall_verdict === 'SOVEREIGNTY_BLOCKED'
          ...
          : r.overall_verdict === 'NEEDS_VERIFICATION'
            ? `<span class="pub-text-xs-secondary">No framework active</span>`
            : r.blockers > 0
