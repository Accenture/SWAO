...
interface RawSpine {
  ...
  overall?: {
    ...
    cloud_native_score?: unknown;
    portability_score?: unknown;
    confidence?: unknown;
...
export async function extractPublicationModel(
  ...
  // migration_rationale extracted from synth pass assessment block (#2702)
  let migrationRationale: string | undefined;
  ...
  if (existsSync(passesDir)) {
    ...
    for (const pf of passFiles) {
      ...
      // Extract migration_rationale from synth pass (#2702)
      if (pf.includes('synth') && typeof passRaw === 'object' && passRaw !== null) {
        const rawSynth = passRaw as Record<string, unknown>;
        const synthAssessment = rawSynth['assessment'] as Record<string, unknown> | undefined;
        const rationale = synthAssessment?.['migration_rationale'];
        if (typeof rationale === 'string' && rationale.trim()) {
          migrationRationale = rationale.trim();
        }
      }
  ...
  const topSignals = [...signals]
    ...
    .slice(0, 3);
  const clamp01 = (v: unknown) => v != null && !isNaN(Number(v)) ? Math.min(1, Math.max(0, Number(v))) : undefined;
  const summary = {
    ...
    cloud_native_score: clamp01(spine.overall?.cloud_native_score),
    portability_score: clamp01(spine.overall?.portability_score),
    ...
    ...(migrationRationale ? { migration_rationale: migrationRationale } : {}),
  };
  ...
  try {
    ...
    if (existsSync(challengeDir)) {
      ...
      // #2681: prefer the latest timestamped subdir (from TUI challenge runs) over
      // combined.yaml (from CLI `swao challenge --report`). combined.yaml is a stale
      // aggregate that is NOT updated by TUI runs; the ts-dir has the freshest data.
      const tsDirs = entries
        .filter(e => e.isDirectory() && /^\d{4}-\d{2}-\d{2}T/.test(e.name))
        .map(e => e.name)
        .sort();
      const latestTsDir = tsDirs.at(-1);
      const effectiveChallengeDir = latestTsDir
        ? join(challengeDir, latestTsDir)
        : challengeDir;
      ...
      for (const cf of challengeFiles) {
        try {
          ...
          if (raw && typeof raw === 'object' && !Array.isArray(raw)) {
            // combined.yaml wraps per-agent reports in reports[].
            // Each entry has agent_id, agent_role, and a `report` YAML string.
            if ('reports' in raw && Array.isArray(raw['reports'])) {
              for (const entry of raw['reports'] as Record<string, unknown>[]) {
                try {
                  const agentId = String(entry['agent_id'] ?? '');
                  const reportStr = typeof entry['report'] === 'string' ? entry['report'] : null;
                  let reportObj: Record<string, unknown> = {};
                  if (reportStr) {
                    const p = load(reportStr);
                    if (p && typeof p === 'object' && !Array.isArray(p)) reportObj = p as Record<string, unknown>;
                  }
                  const rawFindings: unknown[] = Array.isArray(reportObj['findings'])
                    ? (reportObj['findings'] as unknown[])
                    : [];
                  const parsedEntry = (await import('./model.js')).ChallengeAgentReportSchema.safeParse({
                    agent_id: agentId,
                    agent_role: entry['agent_role'],
                    findings: rawFindings,
                  });
                  if (parsedEntry.success) challengeReports.push(parsedEntry.data);
                } catch { /* skip unparseable combined entry */ }
              }
              continue;
            }
